...
async fn test_get_user_success() {
    ...
    let created_user = ctx.user_api.create_user(create_req).await.unwrap().into_inner().user.unwrap();
...
async fn test_update_user_details() {
    ...
    let mut user = ctx.user_api.create_user(create_req).await.unwrap().into_inner().user.unwrap();
    ...
    let update_resp = ctx.user_api.update_user(update_req).await.unwrap().into_inner();
    ...
    let final_user = ctx.user_api.get_user(get_req).await.unwrap().into_inner().user.unwrap();
...
async fn test_delete_user_check() {
    ...
    let user = ctx.user_api.create_user(create_req).await.unwrap().into_inner().user.unwrap();

    // Delete
    let del_req = Request::new(DeleteUserRequest { user_id: user.id.clone() });
    let del_resp = ctx.user_api.delete_user(del_req).await.unwrap().into_inner();
