...
use ledger::proto::ledger::{
    ...
    CreateWalletRequest, RecordOrderRequest,
};
...
async fn test_order_scaling_and_locking() {
    ...
    let btc_resp = ctx
        .asset_api
        .create_asset(Request::new(CreateAssetRequest {
    ...
    ctx.deposit_api
        .create_deposit(Request::new(CreateDepositRequest {
    ...
    let stored_order = ctx
        .order_api
        .get_order_internal(Uuid::parse_str(&order_id).unwrap())
        .await
        .unwrap()
        .unwrap();
