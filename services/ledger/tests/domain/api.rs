...
async fn test_create_and_get_account() {
    ...
    let create_resp = ctx.account_api.create_account(create_req).await.unwrap().into_inner();
    ...
    let get_resp = ctx.account_api.get_account(get_req).await.unwrap().into_inner();
...
async fn test_cancel_order() {
    ...
    let w_locked = ctx.wallet_api.get_wallet(get_wallet_req).await.unwrap().into_inner().wallet.unwrap();
    ...
    let cancel_resp = ctx.order_api.cancel_order(cancel_req).await.unwrap().into_inner();
    ...
    let w_unlocked = ctx.wallet_api.get_wallet(get_wallet_req_2).await.unwrap().into_inner().wallet.unwrap();
