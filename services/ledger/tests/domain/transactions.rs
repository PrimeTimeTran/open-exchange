...
async fn test_deposit_increases_balance() {
    ...
    let acc_id = ctx.create_account_api(&ctx.user_id, "cash").await;
    ...
    let w1 = ctx.wallet_api.get_wallet(get_req).await.unwrap().into_inner().wallet.unwrap();
    ...
    let w2 = ctx.wallet_api.get_wallet(get_req2).await.unwrap().into_inner().wallet.unwrap();
...
async fn test_withdrawal_request_locks_funds() {
    ...
    let resp = ctx.withdrawal_api.create_withdrawal(req).await;
    ...
    if let Ok(r) = resp {
        ...
        // Verify Locked
        let get_req = Request::new(GetWalletRequest { wallet_id: wallet_id.clone() });
        let w = ctx.wallet_api.get_wallet(get_req).await.unwrap().into_inner().wallet.unwrap();
