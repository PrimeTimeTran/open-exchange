...
async fn test_deposit_increases_balance() {
    ...
    let acc_id = create_account(&ctx.service, &ctx.user_id, "cash").await;
    ...
    let w1 = ctx.service.get_wallet(get_req).await.unwrap().into_inner().wallet.unwrap();
    assert_eq!(w1.available, "0");

    // Deposit
    deposit_funds(&ctx.service, &wallet_id, "500").await;

    // Final check
    let get_req2 = Request::new(GetWalletRequest { wallet_id: wallet_id.clone() });
    let w2 = ctx.service.get_wallet(get_req2).await.unwrap().into_inner().wallet.unwrap();
...
async fn test_withdrawal_request_locks_funds() {
    ...
    deposit_funds(&ctx.service, &wallet_id, "1000").await;
    ...
    let resp = ctx.service.create_withdrawal(req).await;
    ...
    if let Ok(r) = resp {
        ...
        // Verify Locked
        let get_req = Request::new(GetWalletRequest { wallet_id: wallet_id.clone() });
        let w = ctx.service.get_wallet(get_req).await.unwrap().into_inner().wallet.unwrap();
