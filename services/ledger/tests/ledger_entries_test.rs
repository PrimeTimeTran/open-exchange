...
async fn test_process_trade_creates_entries() {
    ...
    assert!(
        entry_a_btc.is_some(),
        "Missing Entry: User A receives +1 BTC"
    );

    // 2. User A (Buyer) pays -30,000 USD (3,000,000 cents)
    let entry_a_usd = find_entry(
        ctx.account_a.to_string(),
        "-3000000",
    ...
    assert!(
        entry_a_usd.is_some(),
        "Missing Entry: User A pays -30,000 USD"
    );
    ...
    assert!(
        entry_a_fee.is_some(),
        "Missing Entry: User A pays -30 USD Fee"
    );

    // 4. User B (Seller) pays -1 BTC (-100,000,000 sats)
    let entry_b_btc = find_entry(
        ctx.account_b.to_string(),
        "-100000000",
        &ctx.btc_id.to_string(),
    );
    ...
    assert!(
        entry_b_usd.is_some(),
        "Missing Entry: User B receives +30,000 USD"
    );
    ...
    assert!(
        entry_exchange.is_some(),
        "Missing Entry: Exchange receives +30 USD Fee Revenue"
    );
...
async fn test_ledger_entries_must_balance() {
    ...
    let result = service
        .process_trade(trade, Decimal::ZERO, Decimal::ZERO)
        .await;
    assert!(result.is_ok(), "Process trade failed: {:?}", result.err());
...
async fn test_order_placement_locks_funds() {
    ...
    assert_eq!(
        entries.len(),
        2,
        "Expected 2 ledger entries for order placement"
    );
    ...
    assert!(
        debit_available.is_some(),
        "Missing Entry: Debit Available -3,000,000"
    );

    // Check Credit Locked
    let credit_locked = find_entry("locked", 3000000.0);
    assert!(
        credit_locked.is_some(),
        "Missing Entry: Credit Locked +3,000,000"
    );
