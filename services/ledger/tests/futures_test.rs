...
async fn test_perpetual_positive_funding_rate_long_pays_short() {
    ...
    ctx.create_wallet(ctx.account_a, &ctx.usd_id.to_string(),
        position_notional.to_f64().unwrap(), 0.0, position_notional.to_f64().unwrap()); // long
    ...
    let long_usd  = ctx.wallet_service.get_wallet_by_account_and_asset(&ctx.account_a.to_string(), &ctx.usd_id.to_string()).await.unwrap().unwrap();
    let short_usd = ctx.wallet_service.get_wallet_by_account_and_asset(&ctx.account_b.to_string(), &ctx.usd_id.to_string()).await.unwrap().unwrap();
...
async fn test_perpetual_negative_funding_rate_short_pays_long() {
    ...
    ctx.create_wallet(ctx.account_b, &ctx.usd_id.to_string(),
        position_notional.to_f64().unwrap(), 0.0, position_notional.to_f64().unwrap()); // short
    ...
    assert_eq!(Decimal::from_str(&short_usd.total).unwrap(), position_notional - funding_payment);
...
async fn test_mark_to_market_settlement_credits_profitable_side() {
    ...
    ctx.create_wallet(ctx.account_a, &ctx.usd_id.to_string(),
        entry_price.to_f64().unwrap(), 0.0, entry_price.to_f64().unwrap()); // long
    ctx.create_wallet(ctx.account_b, &ctx.usd_id.to_string(),
        entry_price.to_f64().unwrap(), 0.0, entry_price.to_f64().unwrap()); // short
    ...
    assert_eq!(Decimal::from_str(&long_usd.total).unwrap(),  entry_price + pnl);
    assert_eq!(Decimal::from_str(&short_usd.total).unwrap(), entry_price - pnl);
