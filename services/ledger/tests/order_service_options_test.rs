...
async fn test_sell_call_option_locks_underlying_collateral(
    ...
    let usd_id = ctx.assets.usd;
    let btc_id = ctx.assets.btc;
    ...
    ctx.seed_wallet(ctx.account_a, btc_id, 1.0, 0.0, 1.0).await;
    // Ensure 0 USD
    ...
    let btc_wallet = ctx
        .wallet_service
        .get_wallet_by_account_and_asset(&ctx.account_a.to_string(), &btc_id.to_string())
        .await
...
async fn test_sell_put_option_locks_cash_collateral() -> Result<(), Box<dyn std::error::Error>> {
    ...
    ctx.seed_wallet(ctx.account_a, usd_id, 50_000.0, 0.0, 50_000.0)
        .await;
    // Ensure 0 BTC
    ctx.empty_wallet(ctx.account_a, btc_id);
    ...
    let usd_wallet = ctx
        .wallet_service
        .get_wallet_by_account_and_asset(&ctx.account_a.to_string(), &usd_id.to_string())
        .await
        .expect("Failed to get wallet")
        .expect("Wallet not found");
    assert_eq!(
...
async fn test_sell_put_option_fails_insufficient_collateral(
    ...
    ctx.seed_wallet(ctx.account_a, usd_id, 10_000.0, 0.0, 10_000.0)
        .await;
