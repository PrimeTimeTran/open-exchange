...
async fn test_sell_call_option_locks_underlying_collateral() {
    ...
    ctx.create_wallet_decimal(ctx.account_a, &btc_id, qty_btc_atomic, Decimal::ZERO, qty_btc_atomic);
    // Ensure 0 USD
    ctx.create_wallet_decimal(ctx.account_a, &usd_id, Decimal::ZERO, Decimal::ZERO, Decimal::ZERO);
    ...
    let _created_order = ctx.order_service.create_order(order).await.expect("Order creation failed");

    // 4. Verify BTC Locked
    let btc_wallet = ctx.wallet_service.get_wallet_by_account_and_asset(&ctx.account_a.to_string(), &btc_id).await.unwrap().unwrap();
    // Wallet stores Atomic units. 1.0 BTC standard = 100,000,000 atomic
    assert_eq!(Decimal::from_str(&btc_wallet.locked).unwrap(), qty_btc_atomic);
...
async fn test_sell_put_option_locks_cash_collateral() {
    ...
    ctx.create_wallet_decimal(ctx.account_a, &usd_id, required_collateral_atomic, Decimal::ZERO, required_collateral_atomic);
    // Ensure 0 BTC
    ctx.create_wallet_decimal(ctx.account_a, &btc_id, Decimal::ZERO, Decimal::ZERO, Decimal::ZERO);
    ...
    let usd_wallet = ctx.wallet_service.get_wallet_by_account_and_asset(&ctx.account_a.to_string(), &usd_id).await.unwrap().unwrap();
    assert_eq!(Decimal::from_str(&usd_wallet.locked).unwrap(), required_collateral_atomic, "Should lock Strike * Qty in Cash");
    
    // BTC wallet should remain empty/untouched (or locking 0)
    let btc_wallet = ctx.wallet_service.get_wallet_by_account_and_asset(&ctx.account_a.to_string(), &btc_id).await.unwrap().unwrap();
    assert_eq!(Decimal::from_str(&btc_wallet.locked).unwrap(), Decimal::ZERO);
}
...
async fn test_sell_put_option_fails_insufficient_collateral() {
    ...
    ctx.create_wallet_decimal(ctx.account_a, &usd_id, insufficient_collateral_atomic, Decimal::ZERO, insufficient_collateral_atomic);
