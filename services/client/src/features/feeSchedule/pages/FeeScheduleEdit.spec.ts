...
async function goToFeeScheduleForm(page: Page) {
  ...
  await page.goto(`/fee-schedule/${feeSchedule.id}/edit`);
  await page.waitForURL(`/fee-schedule/${feeSchedule.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToFeeScheduleForm(page);
});
