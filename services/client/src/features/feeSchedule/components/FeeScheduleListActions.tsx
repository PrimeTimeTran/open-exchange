...
import {
  feeScheduleDestroyManyApiCall,
  feeScheduleFindManyApiCall,
  feeScheduleArchiveManyApiCall,
  feeScheduleRestoreManyApiCall,
} from 'src/features/feeSchedule/feeScheduleApiCalls';
import { feeScheduleExporterMapper } from 'src/features/feeSchedule/feeScheduleExporterMapper';
import { feeScheduleFilterInputSchema } from 'src/features/feeSchedule/feeScheduleSchemas';
...
export default function FeeScheduleListActions({
  context,
  table,
  count,
  filter,
  ...
  filter: z.input<typeof feeScheduleFilterInputSchema>;
  sorting: DataTableSort;
  count?: number;
  context: AppContext;
  table: Table<FeeSchedule>;
}) {
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      csvExporter(
        feeScheduleExporterMapper(data.feeSchedules, context),
        dictionary.feeSchedule.fields,
        'feeSchedules',
      );
      toast({
        description: dictionary.feeSchedule.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.feeSchedule.destroyMany.noSelection);
      }

      return feeScheduleDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.feeSchedule.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.feeSchedule.archiveMany.noSelection);
      }

      return feeScheduleArchiveManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.feeSchedule.archiveMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.feeSchedule.restoreMany.noSelection);
      }

      return feeScheduleRestoreManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.feeSchedule.restoreMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/admin/fee-schedule/importer`} prefetch={false}>
      ...
      {archiveManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.feeSchedule.archiveMany.confirmTitle}
          description={formatTranslation(
            dictionary.feeSchedule.archiveMany.confirmDescription,
            selectedCount,
          )}
      ...
      {restoreManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.feeSchedule.restoreMany.confirmTitle}
          description={formatTranslation(
            dictionary.feeSchedule.restoreMany.confirmDescription,
            selectedCount,
          )}
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.feeSchedule.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.feeSchedule.destroyMany.confirmDescription,
            selectedCount,
          )}
