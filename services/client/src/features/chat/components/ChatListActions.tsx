...
import {
  chatDestroyManyApiCall,
  chatFindManyApiCall,
  chatArchiveManyApiCall,
  chatRestoreManyApiCall,
} from 'src/features/chat/chatApiCalls';
import { chatExporterMapper } from 'src/features/chat/chatExporterMapper';
import { chatFilterInputSchema } from 'src/features/chat/chatSchemas';
...
export default function ChatListActions({
  ...
  filter: z.input<typeof chatFilterInputSchema>;
  ...
  const hasPermissionToCreate = hasPermission(permissions.chatCreate, context);
  ...
  const hasPermissionToImport = hasPermission(permissions.chatImport, context);
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      csvExporter(
        chatExporterMapper(data.chats, context),
        dictionary.chat.fields,
        'chats',
      );
      toast({
        description: dictionary.chat.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.chat.destroyMany.noSelection);
      }

      return chatDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.chat.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.chat.archiveMany.noSelection);
      }

      return chatArchiveManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.chat.archiveMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.chat.restoreMany.noSelection);
      }

      return chatRestoreManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.chat.restoreMany.success,
      });
    },
    onError: (error: any) => {
      toast({
    ...
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/admin/chat/importer`} prefetch={false}>
      ...
      {hasPermissionToCreate && (
        <Button
          ...
          asChild
        >
          <Link href={`/admin/chat/new`} prefetch={false}>
      ...
      {archiveManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.chat.archiveMany.confirmTitle}
          description={formatTranslation(
            dictionary.chat.archiveMany.confirmDescription,
            selectedCount,
          )}
      ...
      {restoreManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.chat.restoreMany.confirmTitle}
          description={formatTranslation(
            dictionary.chat.restoreMany.confirmDescription,
            selectedCount,
          )}
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.chat.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.chat.destroyMany.confirmDescription,
            selectedCount,
          )}
