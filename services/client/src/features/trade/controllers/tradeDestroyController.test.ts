import { Trade, Tenant, User } from '@prisma/client';
...
describe('tradeDestroy', () => {
  ...
  it('must be signed in', async () => {
    try {
      await tradeDestroyManyController(
        {
          ids: [trade?.id],
        },
        await testContext(),
  ...
  it.skip('deletes', async () => {
    const countBeforeDelete = await prisma.trade.count();
    ...
    const countAfterDelete = await prisma.trade.count();
