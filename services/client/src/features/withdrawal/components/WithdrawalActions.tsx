...
import { withdrawalDestroyManyApiCall, withdrawalArchiveManyApiCall, withdrawalRestoreManyApiCall } from 'src/features/withdrawal/withdrawalApiCalls';
...
export function WithdrawalActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.withdrawalUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return withdrawalDestroyManyApiCall([withdrawal.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.withdrawal.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      return withdrawalArchiveManyApiCall([withdrawal.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['withdrawal'],
      ...
      toast({
        description: dictionary.withdrawal.archive.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      return withdrawalRestoreManyApiCall([withdrawal.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['withdrawal'],
      ...
      toast({
        description: dictionary.withdrawal.restore.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/withdrawal/${withdrawal.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/withdrawal/${withdrawal.id}/edit`} prefetch={false}>
          ...
          {hasPermissionToArchive && !withdrawal?.archivedAt && (
            <DropdownMenuItem
          ...
          {hasPermissionToRestore && Boolean(withdrawal?.archivedAt) && (
            <DropdownMenuItem
      ...
      {archiveDialogOpen && (
        <ConfirmDialog
          title={dictionary.withdrawal.archive.confirmTitle}
      ...
      {restoreDialogOpen && (
        <ConfirmDialog
          title={dictionary.withdrawal.restore.confirmTitle}
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.withdrawal.destroy.confirmTitle}
