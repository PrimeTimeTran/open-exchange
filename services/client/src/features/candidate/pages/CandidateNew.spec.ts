...
async function goToCandidateNew(page: Page) {
...
test.beforeEach(async ({ page }) => {
  ...
  await goToCandidateNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.candidate.new.title }),
  ).toBeTruthy();
});
