...
async function goToCandidateForm(page: Page) {
  ...
  // TODO: Create candidate here
  // await prisma.candidate.create({});

  await page.waitForURL('/');
  await page.goto(`/candidate/${candidate.id}/edit`);
  await page.waitForURL(`/candidate/${candidate.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToCandidateForm(page);
});
