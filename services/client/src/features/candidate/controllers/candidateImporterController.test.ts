...
import { candidateImporterController } from 'src/features/candidate/controllers/candidateImporterController';
...
async function buildCandidate(context: AppContext) {
...
describe('candidateImport', () => {
  ...
  it('must be signed in', async () => {
    const body = await buildCandidate(
      await testContext({
    ...
    try {
      await candidateImporterController(body, await testContext());
      fail();
    } catch (error: any) {
  ...
  it('must have permission', async () => {
    ...
    try {
      await candidateImporterController(
        body,
        await testContext({
          currentUserId: currentUser?.id,
          currentTenantId: currentTenant?.id,
  ...
  it.skip('imports', async () => {
    ...
    const response = await candidateImporterController(
