...
import { candidateDestroyManyApiCall, candidateArchiveManyApiCall, candidateRestoreManyApiCall } from 'src/features/candidate/candidateApiCalls';
...
export function CandidateActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.candidateUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return candidateDestroyManyApiCall([candidate.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.candidate.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      return candidateArchiveManyApiCall([candidate.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['candidate'],
      ...
      toast({
        description: dictionary.candidate.archive.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      return candidateRestoreManyApiCall([candidate.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['candidate'],
      ...
      toast({
        description: dictionary.candidate.restore.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/candidate/${candidate.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/candidate/${candidate.id}/edit`} prefetch={false}>
          ...
          {hasPermissionToArchive && !candidate?.archivedAt && (
            <DropdownMenuItem
          ...
          {hasPermissionToRestore && Boolean(candidate?.archivedAt) && (
            <DropdownMenuItem
      ...
      {archiveDialogOpen && (
        <ConfirmDialog
          title={dictionary.candidate.archive.confirmTitle}
      ...
      {restoreDialogOpen && (
        <ConfirmDialog
          title={dictionary.candidate.restore.confirmTitle}
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.candidate.destroy.confirmTitle}
