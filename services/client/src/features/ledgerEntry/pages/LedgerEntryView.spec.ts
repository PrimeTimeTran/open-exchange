...
async function goToLedgerEntryView(page: Page) {
  ...
  await page.goto(`/ledger-entry/${ledgerEntry.id}`);
  await page.waitForURL(`/ledger-entry/${ledgerEntry.id}`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToLedgerEntryView(page);
});
