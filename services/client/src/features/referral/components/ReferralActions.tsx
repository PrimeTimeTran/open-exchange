...
import { referralDestroyManyApiCall, referralArchiveManyApiCall, referralRestoreManyApiCall } from 'src/features/referral/referralApiCalls';
...
export function ReferralActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.referralUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return referralDestroyManyApiCall([referral.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.referral.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      return referralArchiveManyApiCall([referral.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['referral'],
      ...
      toast({
        description: dictionary.referral.archive.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      return referralRestoreManyApiCall([referral.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['referral'],
      ...
      toast({
        description: dictionary.referral.restore.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/referral/${referral.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/referral/${referral.id}/edit`} prefetch={false}>
          ...
          {hasPermissionToArchive && !referral?.archivedAt && (
            <DropdownMenuItem
          ...
          {hasPermissionToRestore && Boolean(referral?.archivedAt) && (
            <DropdownMenuItem
      ...
      {archiveDialogOpen && (
        <ConfirmDialog
          title={dictionary.referral.archive.confirmTitle}
      ...
      {restoreDialogOpen && (
        <ConfirmDialog
          title={dictionary.referral.restore.confirmTitle}
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.referral.destroy.confirmTitle}
