...
async function goToReferralNew(page: Page) {
...
test.beforeEach(async ({ page }) => {
  ...
  await goToReferralNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.referral.new.title }),
  ).toBeTruthy();
});
