...
async function goToReferralForm(page: Page) {
  ...
  // TODO: Create referral here
  // await prisma.referral.create({});

  await page.waitForURL('/');
  await page.goto(`/referral/${referral.id}/edit`);
  await page.waitForURL(`/referral/${referral.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToReferralForm(page);
});
