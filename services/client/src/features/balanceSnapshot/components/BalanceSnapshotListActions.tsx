...
import {
  balanceSnapshotDestroyManyApiCall,
  balanceSnapshotFindManyApiCall,
  balanceSnapshotArchiveManyApiCall,
  balanceSnapshotRestoreManyApiCall,
} from 'src/features/balanceSnapshot/balanceSnapshotApiCalls';
import { balanceSnapshotExporterMapper } from 'src/features/balanceSnapshot/balanceSnapshotExporterMapper';
import { balanceSnapshotFilterInputSchema } from 'src/features/balanceSnapshot/balanceSnapshotSchemas';
...
export default function BalanceSnapshotListActions({
  context,
  table,
  count,
  filter,
  ...
  filter: z.input<typeof balanceSnapshotFilterInputSchema>;
  sorting: DataTableSort;
  count?: number;
  context: AppContext;
  table: Table<BalanceSnapshot>;
}) {
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      csvExporter(
        balanceSnapshotExporterMapper(data.balanceSnapshots, context),
        dictionary.balanceSnapshot.fields,
        'balanceSnapshots',
      );
      toast({
        description: dictionary.balanceSnapshot.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        description: dictionary.balanceSnapshot.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        description: dictionary.balanceSnapshot.archiveMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        description: dictionary.balanceSnapshot.restoreMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/admin/balance-snapshot/importer`} prefetch={false}>
      ...
      {archiveManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.balanceSnapshot.archiveMany.confirmTitle}
          description={formatTranslation(
            dictionary.balanceSnapshot.archiveMany.confirmDescription,
            selectedCount,
          )}
      ...
      {restoreManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.balanceSnapshot.restoreMany.confirmTitle}
          description={formatTranslation(
            dictionary.balanceSnapshot.restoreMany.confirmDescription,
            selectedCount,
          )}
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.balanceSnapshot.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.balanceSnapshot.destroyMany.confirmDescription,
            selectedCount,
          )}
