...
async function goToBalanceSnapshotList(page: Page) {
  ...
  // TODO: Create balanceSnapshot here
  // await prisma.balanceSnapshot.create({});

  await page.waitForURL('/');
  await page.goto(`/balance-snapshot`);
  await page.waitForURL(`/balance-snapshot`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToBalanceSnapshotList(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.balanceSnapshot.list.title }),
  ).toBeTruthy();
});
