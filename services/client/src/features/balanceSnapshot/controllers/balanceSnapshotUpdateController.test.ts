...
async function createBalanceSnapshot(context: AppContext): Promise<any> {
...
describe('balanceSnapshotUpdate', () => {
  ...
  it.skip('must be signed in', async () => {
    const balanceSnapshot = await createBalanceSnapshot(
      await testContext({
    ...
    try {
      await balanceSnapshotUpdateController(
  ...
  //     const previousBalanceSnapshot = await createBalanceSnapshot(
  //       await testContext({
  //         currentUserId: currentUser?.id,
  //         currentTenantId: currentTenant?.id,
  //       }),
  ...
  //     const updatedBalanceSnapshot = await prisma.balanceSnapshot.findUniqueOrThrow({
  //       where: { id: previousBalanceSnapshot?.id },
  //       include: {},
  //     });
