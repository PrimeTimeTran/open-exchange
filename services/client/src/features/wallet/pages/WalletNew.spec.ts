...
async function goToWalletNew(page: Page) {
  ...
  await page.goto(`/wallet/new`);
  await page.waitForURL(`/wallet/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToWalletNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.wallet.new.title }),
  ).toBeTruthy();
});
