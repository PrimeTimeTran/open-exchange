...
async function goToInstrumentNew(page: Page) {
  ...
  await page.goto(`/instrument/new`);
  await page.waitForURL(`/instrument/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToInstrumentNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.instrument.new.title }),
  ).toBeTruthy();
});
