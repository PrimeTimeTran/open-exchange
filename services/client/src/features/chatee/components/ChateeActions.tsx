...
import { chateeDestroyManyApiCall, chateeArchiveManyApiCall, chateeRestoreManyApiCall } from 'src/features/chatee/chateeApiCalls';
...
export function ChateeActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.chateeUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return chateeDestroyManyApiCall([chatee.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.chatee.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      return chateeArchiveManyApiCall([chatee.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['chatee'],
      ...
      toast({
        description: dictionary.chatee.archive.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      return chateeRestoreManyApiCall([chatee.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['chatee'],
      ...
      toast({
        description: dictionary.chatee.restore.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/chatee/${chatee.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/chatee/${chatee.id}/edit`} prefetch={false}>
          ...
          {hasPermissionToArchive && !chatee?.archivedAt && (
            <DropdownMenuItem
          ...
          {hasPermissionToRestore && Boolean(chatee?.archivedAt) && (
            <DropdownMenuItem
      ...
      {archiveDialogOpen && (
        <ConfirmDialog
          title={dictionary.chatee.archive.confirmTitle}
      ...
      {restoreDialogOpen && (
        <ConfirmDialog
          title={dictionary.chatee.restore.confirmTitle}
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.chatee.destroy.confirmTitle}
