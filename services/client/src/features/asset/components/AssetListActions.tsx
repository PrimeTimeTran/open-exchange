...
import {
  assetDestroyManyApiCall,
  assetFindManyApiCall,
  assetArchiveManyApiCall,
  assetRestoreManyApiCall,
} from 'src/features/asset/assetApiCalls';
import { assetExporterMapper } from 'src/features/asset/assetExporterMapper';
import { assetFilterInputSchema } from 'src/features/asset/assetSchemas';
...
export default function AssetListActions({
  context,
  table,
  count,
  filter,
  ...
  filter: z.input<typeof assetFilterInputSchema>;
  ...
  const hasPermissionToCreate = hasPermission(permissions.assetCreate, context);
  ...
  const hasPermissionToImport = hasPermission(permissions.assetImport, context);
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      csvExporter(
        assetExporterMapper(data.assets, context),
        dictionary.asset.fields,
        'assets',
      );
      toast({
        description: dictionary.asset.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.asset.destroyMany.noSelection);
      }

      return assetDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.asset.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.asset.archiveMany.noSelection);
      }

      return assetArchiveManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.asset.archiveMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.asset.restoreMany.noSelection);
      }

      return assetRestoreManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.asset.restoreMany.success,
      });
    },
    onError: (error: any) => {
      toast({
    ...
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/admin/asset/importer`} prefetch={false}>
      ...
      {hasPermissionToCreate && (
        <Button
          ...
          asChild
        >
          <Link href={`/admin/admin/asset/new`} prefetch={false}>
      ...
      {archiveManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.asset.archiveMany.confirmTitle}
          description={formatTranslation(
            dictionary.asset.archiveMany.confirmDescription,
            selectedCount,
          )}
      ...
      {restoreManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.asset.restoreMany.confirmTitle}
          description={formatTranslation(
            dictionary.asset.restoreMany.confirmDescription,
            selectedCount,
          )}
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.asset.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.asset.destroyMany.confirmDescription,
            selectedCount,
          )}
