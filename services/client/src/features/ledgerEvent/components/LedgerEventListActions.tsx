...
import {
  ledgerEventDestroyManyApiCall,
  ledgerEventFindManyApiCall,
  ledgerEventArchiveManyApiCall,
  ledgerEventRestoreManyApiCall,
} from 'src/features/ledgerEvent/ledgerEventApiCalls';
import { ledgerEventExporterMapper } from 'src/features/ledgerEvent/ledgerEventExporterMapper';
import { ledgerEventFilterInputSchema } from 'src/features/ledgerEvent/ledgerEventSchemas';
...
export default function LedgerEventListActions({
  context,
  table,
  count,
  filter,
  ...
  filter: z.input<typeof ledgerEventFilterInputSchema>;
  sorting: DataTableSort;
  count?: number;
  context: AppContext;
  table: Table<LedgerEvent>;
}) {
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      csvExporter(
        ledgerEventExporterMapper(data.ledgerEvents, context),
        dictionary.ledgerEvent.fields,
        'ledgerEvents',
      );
      toast({
        description: dictionary.ledgerEvent.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.ledgerEvent.destroyMany.noSelection);
      }

      return ledgerEventDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.ledgerEvent.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.ledgerEvent.archiveMany.noSelection);
      }

      return ledgerEventArchiveManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.ledgerEvent.archiveMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.ledgerEvent.restoreMany.noSelection);
      }

      return ledgerEventRestoreManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.ledgerEvent.restoreMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/admin/ledger-event/importer`} prefetch={false}>
      ...
      {archiveManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.ledgerEvent.archiveMany.confirmTitle}
          description={formatTranslation(
            dictionary.ledgerEvent.archiveMany.confirmDescription,
            selectedCount,
          )}
      ...
      {restoreManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.ledgerEvent.restoreMany.confirmTitle}
          description={formatTranslation(
            dictionary.ledgerEvent.restoreMany.confirmDescription,
            selectedCount,
          )}
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.ledgerEvent.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.ledgerEvent.destroyMany.confirmDescription,
            selectedCount,
          )}
