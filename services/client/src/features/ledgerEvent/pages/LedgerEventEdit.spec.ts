...
async function goToLedgerEventForm(page: Page) {
  ...
  await page.goto(`/ledger-event/${ledgerEvent.id}/edit`);
  await page.waitForURL(`/ledger-event/${ledgerEvent.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToLedgerEventForm(page);
});
