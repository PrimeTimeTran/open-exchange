...
async function goToLedgerEventList(page: Page) {
  ...
  // TODO: Create ledgerEvent here
  // await prisma.ledgerEvent.create({});

  await page.waitForURL('/');
  await page.goto(`/ledger-event`);
  await page.waitForURL(`/ledger-event`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToLedgerEventList(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.ledgerEvent.list.title }),
  ).toBeTruthy();
});
