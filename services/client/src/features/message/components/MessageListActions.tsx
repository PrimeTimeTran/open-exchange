...
import {
  messageDestroyManyApiCall,
  messageFindManyApiCall,
  messageArchiveManyApiCall,
  messageRestoreManyApiCall,
} from 'src/features/message/messageApiCalls';
import { messageExporterMapper } from 'src/features/message/messageExporterMapper';
import { messageFilterInputSchema } from 'src/features/message/messageSchemas';
...
export default function MessageListActions({
  ...
  filter: z.input<typeof messageFilterInputSchema>;
  ...
  const hasPermissionToCreate = hasPermission(permissions.messageCreate, context);
  ...
  const hasPermissionToImport = hasPermission(permissions.messageImport, context);
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      csvExporter(
        messageExporterMapper(data.messages, context),
        dictionary.message.fields,
        'messages',
      );
      toast({
        description: dictionary.message.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.message.destroyMany.noSelection);
      }

      return messageDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.message.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.message.archiveMany.noSelection);
      }

      return messageArchiveManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.message.archiveMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.message.restoreMany.noSelection);
      }

      return messageRestoreManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.message.restoreMany.success,
      });
    },
    onError: (error: any) => {
      toast({
    ...
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/message/importer`} prefetch={false}>
      ...
      {hasPermissionToCreate && (
        <Button
          ...
          asChild
        >
          <Link href={`/message/new`} prefetch={false}>
      ...
      {archiveManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.message.archiveMany.confirmTitle}
          description={formatTranslation(
            dictionary.message.archiveMany.confirmDescription,
            selectedCount,
          )}
      ...
      {restoreManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.message.restoreMany.confirmTitle}
          description={formatTranslation(
            dictionary.message.restoreMany.confirmDescription,
            selectedCount,
          )}
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.message.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.message.destroyMany.confirmDescription,
            selectedCount,
          )}
