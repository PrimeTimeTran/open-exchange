...
async function goToUserNotificationView(page: Page) {
  ...
  await page.goto(`/user-notification/${userNotification.id}`);
  await page.waitForURL(`/user-notification/${userNotification.id}`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToUserNotificationView(page);
});
