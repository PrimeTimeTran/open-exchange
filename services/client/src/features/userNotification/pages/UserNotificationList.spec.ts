...
async function goToUserNotificationList(page: Page) {
  ...
  await page.waitForURL(`/user-notification`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToUserNotificationList(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.userNotification.list.title }),
  ).toBeTruthy();
});
