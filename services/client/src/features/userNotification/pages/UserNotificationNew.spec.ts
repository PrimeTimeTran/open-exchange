...
async function goToUserNotificationNew(page: Page) {
  ...
  await page.goto(`/user-notification/new`);
  await page.waitForURL(`/user-notification/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToUserNotificationNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.userNotification.new.title }),
  ).toBeTruthy();
});
