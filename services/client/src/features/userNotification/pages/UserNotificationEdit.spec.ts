...
async function goToUserNotificationForm(page: Page) {
  ...
  await page.goto(`/user-notification/${userNotification.id}/edit`);
  await page.waitForURL(`/user-notification/${userNotification.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToUserNotificationForm(page);
});
