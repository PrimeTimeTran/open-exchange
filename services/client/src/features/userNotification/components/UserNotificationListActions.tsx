...
import {
  ...
  userNotificationFindManyApiCall,
...
import { userNotificationExporterMapper } from 'src/features/userNotification/userNotificationExporterMapper';
import { userNotificationFilterInputSchema } from 'src/features/userNotification/userNotificationSchemas';
...
export default function UserNotificationListActions({
  context,
  table,
  count,
  filter,
  ...
  filter: z.input<typeof userNotificationFilterInputSchema>;
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      csvExporter(
        userNotificationExporterMapper(data.userNotifications, context),
        dictionary.userNotification.fields,
        'userNotifications',
      );
      toast({
        description: dictionary.userNotification.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        description: dictionary.userNotification.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        description: dictionary.userNotification.archiveMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        description: dictionary.userNotification.restoreMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/admin/user-notification/importer`} prefetch={false}>
      ...
      {archiveManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.userNotification.archiveMany.confirmTitle}
          description={formatTranslation(
            dictionary.userNotification.archiveMany.confirmDescription,
            selectedCount,
          )}
      ...
      {restoreManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.userNotification.restoreMany.confirmTitle}
          description={formatTranslation(
            dictionary.userNotification.restoreMany.confirmDescription,
            selectedCount,
          )}
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.userNotification.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.userNotification.destroyMany.confirmDescription,
            selectedCount,
          )}
