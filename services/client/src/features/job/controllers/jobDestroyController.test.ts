import { Job, Tenant, User } from '@prisma/client';
...
describe('jobDestroy', () => {
  ...
  it('must be signed in', async () => {
    try {
      await jobDestroyManyController(
        {
          ids: [job?.id],
        },
        await testContext(),
  ...
  it.skip('deletes', async () => {
    const countBeforeDelete = await prisma.job.count();
    ...
    const countAfterDelete = await prisma.job.count();
