...
import { jobExporterMapper } from 'src/features/job/jobExporterMapper';
import { jobFilterInputSchema } from 'src/features/job/jobSchemas';
...
export default function JobListActions({
  ...
  filter: z.input<typeof jobFilterInputSchema>;
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      csvExporter(
        jobExporterMapper(data.jobs, context),
        dictionary.job.fields,
        'jobs',
      );
      toast({
        description: dictionary.job.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.job.destroyMany.noSelection);
      }

      return jobDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.job.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.job.archiveMany.noSelection);
      }

      return jobArchiveManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.job.archiveMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.job.restoreMany.noSelection);
      }

      return jobRestoreManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.job.restoreMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <>
      ...
      {archiveManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.job.archiveMany.confirmTitle}
          description={formatTranslation(
            dictionary.job.archiveMany.confirmDescription,
            selectedCount,
          )}
      ...
      {restoreManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.job.restoreMany.confirmTitle}
          description={formatTranslation(
            dictionary.job.restoreMany.confirmDescription,
            selectedCount,
          )}
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.job.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.job.destroyMany.confirmDescription,
            selectedCount,
          )}
