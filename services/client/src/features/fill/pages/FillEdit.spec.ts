...
async function goToFillForm(page: Page) {
  ...
  // TODO: Create fill here
  // await prisma.fill.create({});

  await page.waitForURL('/');
  await page.goto(`/fill/${fill.id}/edit`);
  await page.waitForURL(`/fill/${fill.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToFillForm(page);
});
