...
import {
  fillDestroyManyApiCall,
  fillFindManyApiCall,
  fillArchiveManyApiCall,
  fillRestoreManyApiCall,
} from 'src/features/fill/fillApiCalls';
import { fillExporterMapper } from 'src/features/fill/fillExporterMapper';
import { fillFilterInputSchema } from 'src/features/fill/fillSchemas';
...
export default function FillListActions({
  context,
  table,
  count,
  filter,
  ...
  filter: z.input<typeof fillFilterInputSchema>;
  ...
  const hasPermissionToCreate = hasPermission(permissions.fillCreate, context);
  ...
  const hasPermissionToImport = hasPermission(permissions.fillImport, context);

  const exportMutation = useMutation({
    mutationFn: () => {
      return fillFindManyApiCall({
        filter: filter,
    ...
    onSuccess: (data) => {
      csvExporter(
        fillExporterMapper(data.fills, context),
        dictionary.fill.fields,
        'fills',
      );
      toast({
        description: dictionary.fill.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.fill.destroyMany.noSelection);
      }

      return fillDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.fill.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.fill.archiveMany.noSelection);
      }

      return fillArchiveManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.fill.archiveMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.fill.restoreMany.noSelection);
      }

      return fillRestoreManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.fill.restoreMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/admin/fill/importer`} prefetch={false}>
      ...
      {archiveManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.fill.archiveMany.confirmTitle}
          description={formatTranslation(
            dictionary.fill.archiveMany.confirmDescription,
            selectedCount,
          )}
      ...
      {restoreManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.fill.restoreMany.confirmTitle}
          description={formatTranslation(
            dictionary.fill.restoreMany.confirmDescription,
            selectedCount,
          )}
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.fill.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.fill.destroyMany.confirmDescription,
            selectedCount,
          )}
