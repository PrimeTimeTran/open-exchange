...
async function goToSystemAccountView(page: Page) {
  ...
  await page.goto(`/system-account/${systemAccount.id}`);
  await page.waitForURL(`/system-account/${systemAccount.id}`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToSystemAccountView(page);
});
