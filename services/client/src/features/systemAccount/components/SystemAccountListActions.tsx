...
import {
  systemAccountDestroyManyApiCall,
  systemAccountFindManyApiCall,
  systemAccountArchiveManyApiCall,
  systemAccountRestoreManyApiCall,
} from 'src/features/systemAccount/systemAccountApiCalls';
import { systemAccountExporterMapper } from 'src/features/systemAccount/systemAccountExporterMapper';
import { systemAccountFilterInputSchema } from 'src/features/systemAccount/systemAccountSchemas';
...
export default function SystemAccountListActions({
  context,
  table,
  count,
  filter,
  ...
  filter: z.input<typeof systemAccountFilterInputSchema>;
  sorting: DataTableSort;
  count?: number;
  context: AppContext;
  table: Table<SystemAccount>;
}) {
  ...
  const hasPermissionToRestore = hasPermission(
    permissions.systemAccountRestore,
    context,
  );
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      csvExporter(
        systemAccountExporterMapper(data.systemAccounts, context),
        dictionary.systemAccount.fields,
        'systemAccounts',
      );
      toast({
        description: dictionary.systemAccount.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.resetQueries({
        queryKey: ['systemAccount'],
      });
      toast({
        description: dictionary.systemAccount.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        description: dictionary.systemAccount.archiveMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        description: dictionary.systemAccount.restoreMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <>
      ...
      {archiveManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.systemAccount.archiveMany.confirmTitle}
          description={formatTranslation(
            dictionary.systemAccount.archiveMany.confirmDescription,
            selectedCount,
          )}
      ...
      {restoreManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.systemAccount.restoreMany.confirmTitle}
          description={formatTranslation(
            dictionary.systemAccount.restoreMany.confirmDescription,
            selectedCount,
          )}
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.systemAccount.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.systemAccount.destroyMany.confirmDescription,
            selectedCount,
          )}
