...
import {
  chaterDestroyManyApiCall,
  chaterFindManyApiCall,
  chaterArchiveManyApiCall,
  chaterRestoreManyApiCall,
} from 'src/features/chater/chaterApiCalls';
import { chaterExporterMapper } from 'src/features/chater/chaterExporterMapper';
import { chaterFilterInputSchema } from 'src/features/chater/chaterSchemas';
...
export default function ChaterListActions({
  context,
  table,
  count,
  filter,
  ...
  filter: z.input<typeof chaterFilterInputSchema>;
  ...
  const hasPermissionToCreate = hasPermission(permissions.chaterCreate, context);
  ...
  const hasPermissionToImport = hasPermission(permissions.chaterImport, context);
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      csvExporter(
        chaterExporterMapper(data.chaters, context),
        dictionary.chater.fields,
        'chaters',
      );
      toast({
        description: dictionary.chater.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.chater.destroyMany.noSelection);
      }

      return chaterDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.chater.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.chater.archiveMany.noSelection);
      }

      return chaterArchiveManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.chater.archiveMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.chater.restoreMany.noSelection);
      }

      return chaterRestoreManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.chater.restoreMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/chater/importer`} prefetch={false}>
      ...
      {archiveManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.chater.archiveMany.confirmTitle}
          description={formatTranslation(
            dictionary.chater.archiveMany.confirmDescription,
            selectedCount,
          )}
      ...
      {restoreManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.chater.restoreMany.confirmTitle}
          description={formatTranslation(
            dictionary.chater.restoreMany.confirmDescription,
            selectedCount,
          )}
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.chater.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.chater.destroyMany.confirmDescription,
            selectedCount,
          )}
