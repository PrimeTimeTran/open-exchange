...
import { chaterDestroyManyApiCall, chaterArchiveManyApiCall, chaterRestoreManyApiCall } from 'src/features/chater/chaterApiCalls';
...
export function ChaterActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.chaterUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return chaterDestroyManyApiCall([chater.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.chater.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      return chaterArchiveManyApiCall([chater.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['chater'],
      ...
      toast({
        description: dictionary.chater.archive.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      return chaterRestoreManyApiCall([chater.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['chater'],
      ...
      toast({
        description: dictionary.chater.restore.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/chater/${chater.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/chater/${chater.id}/edit`} prefetch={false}>
          ...
          {hasPermissionToArchive && !chater?.archivedAt && (
            <DropdownMenuItem
          ...
          {hasPermissionToRestore && Boolean(chater?.archivedAt) && (
            <DropdownMenuItem
      ...
      {archiveDialogOpen && (
        <ConfirmDialog
          title={dictionary.chater.archive.confirmTitle}
      ...
      {restoreDialogOpen && (
        <ConfirmDialog
          title={dictionary.chater.restore.confirmTitle}
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.chater.destroy.confirmTitle}
