...
import { chaterUpdateController } from 'src/features/chater/controllers/chaterUpdateController';
...
describe('chaterUpdate', () => {
  ...
  it.skip('must be signed in', async () => {
    const chater = await createChater(
      await testContext({
  ...
  //     const previousChater = await createChater(
  //       await testContext({
  //         currentUserId: currentUser?.id,
  //         currentTenantId: currentTenant?.id,
  //       }),
  ...
  //     const updatedChater = await prisma.chater.findUniqueOrThrow({
  //       where: { id: previousChater?.id },
  //       include: {},
  //     });
