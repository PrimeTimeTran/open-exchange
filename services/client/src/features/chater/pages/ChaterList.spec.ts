...
async function goToChaterList(page: Page) {
  ...
  await page.goto(`/chater`);
  await page.waitForURL(`/chater`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToChaterList(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.chater.list.title }),
  ).toBeTruthy();
});
