...
async function goToChaterNew(page: Page) {
  ...
  await page.goto(`/chater/new`);
  await page.waitForURL(`/chater/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToChaterNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.chater.new.title }),
  ).toBeTruthy();
});
