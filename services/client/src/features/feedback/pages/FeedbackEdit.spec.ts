...
async function goToFeedbackForm(page: Page) {
  ...
  // TODO: Create feedback here
  // await prisma.feedback.create({});

  await page.waitForURL('/');
  await page.goto(`/feedback/${feedback.id}/edit`);
  await page.waitForURL(`/feedback/${feedback.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToFeedbackForm(page);
});
