...
import { postDestroyManyApiCall, postArchiveManyApiCall, postRestoreManyApiCall } from 'src/features/post/postApiCalls';
...
export function PostActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.postUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return postDestroyManyApiCall([post.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.post.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      return postArchiveManyApiCall([post.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['post'],
      ...
      toast({
        description: dictionary.post.archive.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      return postRestoreManyApiCall([post.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['post'],
      ...
      toast({
        description: dictionary.post.restore.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/post/${post.id}/edit`} prefetch={false}>
          ...
          {hasPermissionToArchive && !post?.archivedAt && (
            <DropdownMenuItem
          ...
          {hasPermissionToRestore && Boolean(post?.archivedAt) && (
            <DropdownMenuItem
      ...
      {archiveDialogOpen && (
        <ConfirmDialog
          title={dictionary.post.archive.confirmTitle}
      ...
      {restoreDialogOpen && (
        <ConfirmDialog
          title={dictionary.post.restore.confirmTitle}
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.post.destroy.confirmTitle}
