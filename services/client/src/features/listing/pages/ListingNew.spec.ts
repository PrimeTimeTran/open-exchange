...
async function goToListingNew(page: Page) {
  ...
  await page.waitForURL(`/listing/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToListingNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.listing.new.title }),
  ).toBeTruthy();
});
