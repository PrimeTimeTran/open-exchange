...
async function goToListingForm(page: Page) {
  ...
  await page.goto(`/listing/${listing.id}/edit`);
  await page.waitForURL(`/listing/${listing.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToListingForm(page);
});
