...
import {
  listingDestroyManyApiCall,
  listingFindManyApiCall,
  listingArchiveManyApiCall,
  listingRestoreManyApiCall,
} from 'src/features/listing/listingApiCalls';
import { listingExporterMapper } from 'src/features/listing/listingExporterMapper';
import { listingFilterInputSchema } from 'src/features/listing/listingSchemas';
...
export default function ListingListActions({
  context,
  table,
  count,
  filter,
  ...
  filter: z.input<typeof listingFilterInputSchema>;
  ...
  const hasPermissionToCreate = hasPermission(permissions.listingCreate, context);
  ...
  const hasPermissionToImport = hasPermission(permissions.listingImport, context);
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      csvExporter(
        listingExporterMapper(data.listings, context),
        dictionary.listing.fields,
        'listings',
      );
      toast({
        description: dictionary.listing.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.listing.destroyMany.noSelection);
      }

      return listingDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.listing.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.listing.archiveMany.noSelection);
      }

      return listingArchiveManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.listing.archiveMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.listing.restoreMany.noSelection);
      }

      return listingRestoreManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.listing.restoreMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/listing/importer`} prefetch={false}>
      ...
      {archiveManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.listing.archiveMany.confirmTitle}
          description={formatTranslation(
            dictionary.listing.archiveMany.confirmDescription,
            selectedCount,
          )}
      ...
      {restoreManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.listing.restoreMany.confirmTitle}
          description={formatTranslation(
            dictionary.listing.restoreMany.confirmDescription,
            selectedCount,
          )}
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.listing.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.listing.destroyMany.confirmDescription,
            selectedCount,
          )}
