...
import { listingDestroyManyApiCall, listingArchiveManyApiCall, listingRestoreManyApiCall } from 'src/features/listing/listingApiCalls';
...
export function ListingActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.listingUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return listingDestroyManyApiCall([listing.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.listing.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      return listingArchiveManyApiCall([listing.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['listing'],
      ...
      toast({
        description: dictionary.listing.archive.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      return listingRestoreManyApiCall([listing.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['listing'],
      ...
      toast({
        description: dictionary.listing.restore.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/listing/${listing.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/listing/${listing.id}/edit`} prefetch={false}>
          ...
          {hasPermissionToArchive && !listing?.archivedAt && (
            <DropdownMenuItem
          ...
          {hasPermissionToRestore && Boolean(listing?.archivedAt) && (
            <DropdownMenuItem
      ...
      {archiveDialogOpen && (
        <ConfirmDialog
          title={dictionary.listing.archive.confirmTitle}
      ...
      {restoreDialogOpen && (
        <ConfirmDialog
          title={dictionary.listing.restore.confirmTitle}
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.listing.destroy.confirmTitle}
