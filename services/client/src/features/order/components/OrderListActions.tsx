...
import {
  orderDestroyManyApiCall,
  orderFindManyApiCall,
  orderArchiveManyApiCall,
  orderRestoreManyApiCall,
} from 'src/features/order/orderApiCalls';
import { orderExporterMapper } from 'src/features/order/orderExporterMapper';
import { orderFilterInputSchema } from 'src/features/order/orderSchemas';
...
export default function OrderListActions({
  ...
  filter: z.input<typeof orderFilterInputSchema>;
  ...
  const hasPermissionToCreate = hasPermission(permissions.orderCreate, context);
  ...
  const hasPermissionToImport = hasPermission(permissions.orderImport, context);
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      csvExporter(
        orderExporterMapper(data.orders, context),
        dictionary.order.fields,
        'orders',
      );
      toast({
        description: dictionary.order.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.order.destroyMany.noSelection);
      }

      return orderDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.order.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.order.archiveMany.noSelection);
      }

      return orderArchiveManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.order.archiveMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.order.restoreMany.noSelection);
      }

      return orderRestoreManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.order.restoreMany.success,
      });
    },
    onError: (error: any) => {
      toast({
    ...
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/admin/order/importer`} prefetch={false}>
      ...
      {hasPermissionToCreate && (
        <Button
          ...
          asChild
        >
          <Link href={`/admin/order/new`} prefetch={false}>
      ...
      {archiveManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.order.archiveMany.confirmTitle}
          description={formatTranslation(
            dictionary.order.archiveMany.confirmDescription,
            selectedCount,
          )}
      ...
      {restoreManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.order.restoreMany.confirmTitle}
          description={formatTranslation(
            dictionary.order.restoreMany.confirmDescription,
            selectedCount,
          )}
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.order.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.order.destroyMany.confirmDescription,
            selectedCount,
          )}
