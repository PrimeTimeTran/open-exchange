...
import {
  notificationDestroyManyApiCall,
  notificationFindManyApiCall,
  notificationArchiveManyApiCall,
  notificationRestoreManyApiCall,
} from 'src/features/notification/notificationApiCalls';
import { notificationExporterMapper } from 'src/features/notification/notificationExporterMapper';
import { notificationFilterInputSchema } from 'src/features/notification/notificationSchemas';
...
export default function NotificationListActions({
  context,
  table,
  count,
  filter,
  ...
  filter: z.input<typeof notificationFilterInputSchema>;
  ...
  const hasPermissionToCreate = hasPermission(permissions.notificationCreate, context);
  ...
  const hasPermissionToImport = hasPermission(permissions.notificationImport, context);
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      csvExporter(
        notificationExporterMapper(data.notifications, context),
        dictionary.notification.fields,
        'notifications',
      );
      toast({
        description: dictionary.notification.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.notification.destroyMany.noSelection);
      }

      return notificationDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.notification.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.notification.archiveMany.noSelection);
      }

      return notificationArchiveManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.notification.archiveMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.notification.restoreMany.noSelection);
      }

      return notificationRestoreManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.notification.restoreMany.success,
      });
    },
    onError: (error: any) => {
      toast({
    ...
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/notification/importer`} prefetch={false}>
      ...
      {hasPermissionToCreate && (
        <Button
          ...
          asChild
        >
          <Link href={`/notification/new`} prefetch={false}>
      ...
      {archiveManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.notification.archiveMany.confirmTitle}
          description={formatTranslation(
            dictionary.notification.archiveMany.confirmDescription,
            selectedCount,
          )}
      ...
      {restoreManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.notification.restoreMany.confirmTitle}
          description={formatTranslation(
            dictionary.notification.restoreMany.confirmDescription,
            selectedCount,
          )}
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.notification.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.notification.destroyMany.confirmDescription,
            selectedCount,
          )}
