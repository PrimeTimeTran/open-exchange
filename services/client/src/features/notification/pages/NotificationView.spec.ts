...
async function goToNotificationView(page: Page) {
  ...
  await page.goto(`/notification/${notification.id}`);
  await page.waitForURL(`/notification/${notification.id}`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToNotificationView(page);
});
