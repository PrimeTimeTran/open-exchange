...
async function goToNotificationForm(page: Page) {
  ...
  await page.goto(`/notification/${notification.id}/edit`);
  await page.waitForURL(`/notification/${notification.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToNotificationForm(page);
});
