...
async function goToNotificationNew(page: Page) {
  ...
  await page.goto(`/notification/new`);
  await page.waitForURL(`/notification/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToNotificationNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.notification.new.title }),
  ).toBeTruthy();
});
