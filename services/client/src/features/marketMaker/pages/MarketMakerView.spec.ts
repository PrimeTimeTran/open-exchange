...
async function goToMarketMakerView(page: Page) {
  ...
  await page.goto(`/market-maker/${marketMaker.id}`);
  await page.waitForURL(`/market-maker/${marketMaker.id}`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToMarketMakerView(page);
});
