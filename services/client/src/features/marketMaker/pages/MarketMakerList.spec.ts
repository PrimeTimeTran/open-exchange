...
async function goToMarketMakerList(page: Page) {
  ...
  await page.goto(`/market-maker`);
  await page.waitForURL(`/market-maker`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToMarketMakerList(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.marketMaker.list.title }),
  ).toBeTruthy();
});
