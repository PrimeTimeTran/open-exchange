...
async function goToMarketMakerNew(page: Page) {
  ...
  await page.goto(`/market-maker/new`);
  await page.waitForURL(`/market-maker/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToMarketMakerNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.marketMaker.new.title }),
  ).toBeTruthy();
});
