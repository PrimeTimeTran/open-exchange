...
async function goToMarketMakerForm(page: Page) {
  ...
  const marketMaker = await prisma.marketMaker.findFirstOrThrow();

  // TODO: Create marketMaker here
  // await prisma.marketMaker.create({});

  await page.waitForURL('/');
  await page.goto(`/market-maker/${marketMaker.id}/edit`);
  await page.waitForURL(`/market-maker/${marketMaker.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToMarketMakerForm(page);
});
