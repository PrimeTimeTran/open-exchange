...
import { marketMakerUpdateController } from 'src/features/marketMaker/controllers/marketMakerUpdateController';
...
async function createMarketMaker(context: AppContext): Promise<any> {
...
describe('marketMakerUpdate', () => {
  ...
  it.skip('must be signed in', async () => {
    const marketMaker = await createMarketMaker(
      await testContext({
    ...
    try {
      await marketMakerUpdateController(
        { id: marketMaker.id },
        {},
        await testContext(),
      );
  ...
  //     const previousMarketMaker = await createMarketMaker(
  //       await testContext({
  //         currentUserId: currentUser?.id,
  //         currentTenantId: currentTenant?.id,
  //       }),
  ...
  //     const updatedMarketMaker = await prisma.marketMaker.findUniqueOrThrow({
  //       where: { id: previousMarketMaker?.id },
  //       include: {},
  //     });
