...
import { marketMakerDestroyManyApiCall, marketMakerArchiveManyApiCall, marketMakerRestoreManyApiCall } from 'src/features/marketMaker/marketMakerApiCalls';
...
export function MarketMakerActions({
  mode,
  marketMaker,
  context,
}: {
  ...
  const hasPermissionToEdit = hasPermission(permissions.marketMakerUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return marketMakerDestroyManyApiCall([marketMaker.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.marketMaker.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      return marketMakerArchiveManyApiCall([marketMaker.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['marketMaker'],
      ...
      toast({
        description: dictionary.marketMaker.archive.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      return marketMakerRestoreManyApiCall([marketMaker.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['marketMaker'],
      ...
      toast({
        description: dictionary.marketMaker.restore.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/market-maker/${marketMaker.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/market-maker/${marketMaker.id}/edit`} prefetch={false}>
          ...
          {hasPermissionToArchive && !marketMaker?.archivedAt && (
            <DropdownMenuItem
          ...
          {hasPermissionToRestore && Boolean(marketMaker?.archivedAt) && (
            <DropdownMenuItem
      ...
      {archiveDialogOpen && (
        <ConfirmDialog
          title={dictionary.marketMaker.archive.confirmTitle}
      ...
      {restoreDialogOpen && (
        <ConfirmDialog
          title={dictionary.marketMaker.restore.confirmTitle}
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.marketMaker.destroy.confirmTitle}
