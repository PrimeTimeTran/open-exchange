...
import {
  marketMakerDestroyManyApiCall,
  marketMakerFindManyApiCall,
  marketMakerArchiveManyApiCall,
  marketMakerRestoreManyApiCall,
} from 'src/features/marketMaker/marketMakerApiCalls';
import { marketMakerExporterMapper } from 'src/features/marketMaker/marketMakerExporterMapper';
import { marketMakerFilterInputSchema } from 'src/features/marketMaker/marketMakerSchemas';
...
export default function MarketMakerListActions({
  context,
  table,
  count,
  filter,
  ...
  filter: z.input<typeof marketMakerFilterInputSchema>;
  sorting: DataTableSort;
  count?: number;
  context: AppContext;
  table: Table<MarketMaker>;
}) {
  ...
  const hasPermissionToCreate = hasPermission(permissions.marketMakerCreate, context);
  ...
  const hasPermissionToImport = hasPermission(permissions.marketMakerImport, context);
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      csvExporter(
        marketMakerExporterMapper(data.marketMakers, context),
        dictionary.marketMaker.fields,
        'marketMakers',
      );
      toast({
        description: dictionary.marketMaker.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.marketMaker.destroyMany.noSelection);
      }

      return marketMakerDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.marketMaker.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.marketMaker.archiveMany.noSelection);
      }

      return marketMakerArchiveManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.marketMaker.archiveMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.marketMaker.restoreMany.noSelection);
      }

      return marketMakerRestoreManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.marketMaker.restoreMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/market-maker/importer`} prefetch={false}>
      ...
      {archiveManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.marketMaker.archiveMany.confirmTitle}
          description={formatTranslation(
            dictionary.marketMaker.archiveMany.confirmDescription,
            selectedCount,
          )}
      ...
      {restoreManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.marketMaker.restoreMany.confirmTitle}
          description={formatTranslation(
            dictionary.marketMaker.restoreMany.confirmDescription,
            selectedCount,
          )}
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.marketMaker.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.marketMaker.destroyMany.confirmDescription,
            selectedCount,
          )}
