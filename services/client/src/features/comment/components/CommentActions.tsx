...
export function CommentActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.commentUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return commentDestroyManyApiCall([comment.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.comment.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      return commentArchiveManyApiCall([comment.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['comment'],
      ...
      toast({
        description: dictionary.comment.archive.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      return commentRestoreManyApiCall([comment.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['comment'],
      ...
      toast({
        description: dictionary.comment.restore.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/admin/comment/${comment.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/admin/comment/${comment.id}/edit`} prefetch={false}>
          ...
          {hasPermissionToArchive && !comment?.archivedAt && (
            <DropdownMenuItem
          ...
          {hasPermissionToRestore && Boolean(comment?.archivedAt) && (
            <DropdownMenuItem
      ...
      {archiveDialogOpen && (
        <ConfirmDialog
          title={dictionary.comment.archive.confirmTitle}
      ...
      {restoreDialogOpen && (
        <ConfirmDialog
          title={dictionary.comment.restore.confirmTitle}
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.comment.destroy.confirmTitle}
