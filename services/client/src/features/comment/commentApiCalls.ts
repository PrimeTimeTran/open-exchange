...
export async function commentArchiveManyApiCall(
  ids: string[],
  signal?: AbortSignal,
) {
  const response = await fetch(
    `${process.env.NEXT_PUBLIC_BACKEND_URL || ''}/api/comment/archive?${objectToQuery({
      ids,
    })}`,
    {
      method: 'PUT',
...
export async function commentRestoreManyApiCall(
  ids: string[],
  signal?: AbortSignal,
) {
  const response = await fetch(
    `${
      process.env.NEXT_PUBLIC_BACKEND_URL || ''
    }/api/comment/restore?${objectToQuery({ ids })}`,
    {
      method: 'PUT',
      signal,
    },
