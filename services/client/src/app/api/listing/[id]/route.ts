...
import { listingUpdateController } from 'src/features/listing/controllers/listingUpdateController';
...
export async function GET(
  ...
  try {
    ...
    const payload = await listingFindController(params, context);
...
export async function PUT(
  ...
  try {
    ...
    const payload = await listingUpdateController(params, body, context);
    return NextResponseSuccess(request, context, payload);
  } catch (error: any) {
    return NextResponseError(request, context, error);
  }
