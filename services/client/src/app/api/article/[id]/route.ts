...
import { articleFindController } from 'src/features/article/controllers/articleFindController';
...
export async function GET(
  ...
  try {
    ...
    const payload = await articleFindController(params, context);
...
export async function PUT(
  ...
  try {
    ...
    const payload = await articleUpdateController(params, body, context);
    return NextResponseSuccess(request, context, payload);
  } catch (error: any) {
    return NextResponseError(request, context, error);
  }
