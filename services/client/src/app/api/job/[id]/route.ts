...
import { jobFindController } from 'src/features/job/controllers/jobFindController';
...
export async function GET(
  ...
  try {
    ...
    const payload = await jobFindController(params, context);
...
export async function PUT(
  ...
  try {
    ...
    const payload = await jobUpdateController(params, body, context);
    return NextResponseSuccess(request, context, payload);
  } catch (error: any) {
    return NextResponseError(request, context, error);
  }
