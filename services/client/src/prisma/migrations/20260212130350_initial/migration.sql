/*
  ...
  - You are about to alter the column `price` on the `Fill` table. The data in that column could be lost. The data in that column will be cast from `Decimal(72,30)` to `Decimal(72,20)`.
  - You are about to alter the column `price` on the `Order` table. The data in that column could be lost. The data in that column will be cast from `Decimal(72,30)` to `Decimal(72,20)`.
  - You are about to alter the column `price` on the `Trade` table. The data in that column could be lost. The data in that column will be cast from `Decimal(72,30)` to `Decimal(72,20)`.
...
ALTER COLUMN "updatedByUserId" SET DEFAULT NULLIF((current_setting('app.current_user_id'::text)), '')::uuid,
ALTER COLUMN "updatedByMembershipId" SET DEFAULT NULLIF((current_setting('app.current_membership_id'::text)), '')::uuid,
ALTER COLUMN "price" SET DATA TYPE DECIMAL(72,20);
