...
import { chatFilterFormSchema } from 'src/features/chat/chatSchemas';
...
function ChatListFilter({
  ...
  const previewRenders = {
    name: {
      label: dictionary.chat.fields.name,
    },
    active: {
      label: dictionary.chat.fields.active,
  ...
  const filter = useMemo(
    () =>
      DataTableQueryParams.getFilter<z.infer<typeof chatFilterFormSchema>>(
        searchParams,
        chatFilterFormSchema,
      ),
    [searchParams],
  );

  const form = useForm({
    resolver: zodResolver(chatFilterFormSchema),
    mode: 'onSubmit',
          ...
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.chat.fields.name}</FormLabel>
