...
import {
  feedbackDestroyManyApiCall,
  feedbackFindManyApiCall,
  feedbackArchiveManyApiCall,
  feedbackRestoreManyApiCall,
} from 'src/features/feedback/feedbackApiCalls';
import { feedbackExporterMapper } from 'src/features/feedback/feedbackExporterMapper';
import { feedbackFilterInputSchema } from 'src/features/feedback/feedbackSchemas';
...
export default function FeedbackListActions({
  context,
  table,
  count,
  filter,
  ...
  filter: z.input<typeof feedbackFilterInputSchema>;
  ...
  const hasPermissionToCreate = hasPermission(permissions.feedbackCreate, context);
  ...
  const hasPermissionToImport = hasPermission(permissions.feedbackImport, context);
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      csvExporter(
        feedbackExporterMapper(data.feedbacks, context),
        dictionary.feedback.fields,
        'feedbacks',
      );
      toast({
        description: dictionary.feedback.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.feedback.destroyMany.noSelection);
      }

      return feedbackDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.feedback.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.feedback.archiveMany.noSelection);
      }

      return feedbackArchiveManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.feedback.archiveMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.feedback.restoreMany.noSelection);
      }

      return feedbackRestoreManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.feedback.restoreMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/feedback/importer`} prefetch={false}>
      ...
      {archiveManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.feedback.archiveMany.confirmTitle}
          description={formatTranslation(
            dictionary.feedback.archiveMany.confirmDescription,
            selectedCount,
          )}
      ...
      {restoreManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.feedback.restoreMany.confirmTitle}
          description={formatTranslation(
            dictionary.feedback.restoreMany.confirmDescription,
            selectedCount,
          )}
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.feedback.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.feedback.destroyMany.confirmDescription,
            selectedCount,
          )}
