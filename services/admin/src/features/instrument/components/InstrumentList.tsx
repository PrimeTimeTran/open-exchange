...
import InstrumentListActions from 'src/features/instrument/components/InstrumentListActions';
import InstrumentListFilter from 'src/features/instrument/components/InstrumentListFilter';
import { instrumentFindManyApiCall } from 'src/features/instrument/instrumentApiCalls';
import {
  InstrumentWithRelationships,
  instrumentFilterInputSchema,
...
import { InstrumentNewButton } from 'src/features/instrument/components/InstrumentNewButton';
import { z } from 'zod';
...
export default function InstrumentList({ context }: { context: AppContext }) {
  ...
  const filter = useMemo(() => {
    return DataTableQueryParams.getFilter<
      z.input<typeof instrumentFilterInputSchema>
    >(searchParams, instrumentFilterInputSchema);
  }, [searchParams]);

  const columns: ColumnDef<InstrumentWithRelationships>[] = [
    {
    ...
    },
    {
      accessorKey: 'status',
      meta: {
        title: dictionary.instrument.fields.status,
      },
      cell: ({ row }) => {
        return enumeratorLabel(
  ...
  const query = useQuery({
    queryKey: ['instrument', 'list', filter, sorting, pagination],
  ...
  const table = useReactTable({
    ...
    data: query.data?.instruments || defaultData,
    columns,
  ...
  return (
    <div className="mb-4 flex w-full flex-col gap-4">
      <div className="flex items-center justify-between">
        <Breadcrumb items={[[dictionary.instrument.list.menu]]} />
      ...
      <InstrumentListFilter context={context} isLoading={query.isLoading} />
      ...
      <DataTable
        ...
        notFoundText={dictionary.instrument.list.noResults}
        newButton={<InstrumentNewButton context={context} />}
      />
