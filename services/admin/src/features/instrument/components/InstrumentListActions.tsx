...
import {
  instrumentDestroyManyApiCall,
  instrumentFindManyApiCall,
  instrumentArchiveManyApiCall,
  instrumentRestoreManyApiCall,
} from 'src/features/instrument/instrumentApiCalls';
import { instrumentExporterMapper } from 'src/features/instrument/instrumentExporterMapper';
import { instrumentFilterInputSchema } from 'src/features/instrument/instrumentSchemas';
...
export default function InstrumentListActions({
  context,
  table,
  count,
  filter,
  ...
  filter: z.input<typeof instrumentFilterInputSchema>;
  ...
  const hasPermissionToCreate = hasPermission(permissions.instrumentCreate, context);
  ...
  const hasPermissionToImport = hasPermission(permissions.instrumentImport, context);

  const exportMutation = useMutation({
    mutationFn: () => {
      return instrumentFindManyApiCall({
        filter: filter,
    ...
    onSuccess: (data) => {
      csvExporter(
        instrumentExporterMapper(data.instruments, context),
        dictionary.instrument.fields,
        'instruments',
      );
      toast({
        description: dictionary.instrument.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.instrument.destroyMany.noSelection);
      }

      return instrumentDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.instrument.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.instrument.archiveMany.noSelection);
      }

      return instrumentArchiveManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.instrument.archiveMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.instrument.restoreMany.noSelection);
      }

      return instrumentRestoreManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.instrument.restoreMany.success,
      });
    },
    onError: (error: any) => {
      toast({
    ...
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/instrument/importer`} prefetch={false}>
      ...
      {hasPermissionToCreate && (
        <Button
          ...
          asChild
        >
          <Link href={`/instrument/new`} prefetch={false}>
      ...
      {archiveManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.instrument.archiveMany.confirmTitle}
          description={formatTranslation(
            dictionary.instrument.archiveMany.confirmDescription,
            selectedCount,
          )}
      ...
      {restoreManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.instrument.restoreMany.confirmTitle}
          description={formatTranslation(
            dictionary.instrument.restoreMany.confirmDescription,
            selectedCount,
          )}
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.instrument.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.instrument.destroyMany.confirmDescription,
            selectedCount,
          )}
