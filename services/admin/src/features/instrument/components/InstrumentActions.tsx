...
import { instrumentDestroyManyApiCall, instrumentArchiveManyApiCall, instrumentRestoreManyApiCall } from 'src/features/instrument/instrumentApiCalls';
...
export function InstrumentActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.instrumentUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return instrumentDestroyManyApiCall([instrument.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.instrument.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      return instrumentArchiveManyApiCall([instrument.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['instrument'],
      ...
      toast({
        description: dictionary.instrument.archive.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      return instrumentRestoreManyApiCall([instrument.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['instrument'],
      ...
      toast({
        description: dictionary.instrument.restore.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/instrument/${instrument.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/instrument/${instrument.id}/edit`} prefetch={false}>
          ...
          {hasPermissionToArchive && !instrument?.archivedAt && (
            <DropdownMenuItem
          ...
          {hasPermissionToRestore && Boolean(instrument?.archivedAt) && (
            <DropdownMenuItem
      ...
      {archiveDialogOpen && (
        <ConfirmDialog
          title={dictionary.instrument.archive.confirmTitle}
      ...
      {restoreDialogOpen && (
        <ConfirmDialog
          title={dictionary.instrument.restore.confirmTitle}
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.instrument.destroy.confirmTitle}
