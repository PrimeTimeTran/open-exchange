...
import { postFilterFormSchema } from 'src/features/post/postSchemas';
...
function PostListFilter({
  ...
  const previewRenders = {
    title: {
      label: dictionary.post.fields.title,
    },
    body: {
      label: dictionary.post.fields.body,
    },
    type: {
      label: dictionary.post.fields.type,
      render: dataTableFilterRenders(context).stringArray(),
    },
    user: {
      label: dictionary.post.fields.user,
  ...
  const filter = useMemo(
    () =>
      DataTableQueryParams.getFilter<z.infer<typeof postFilterFormSchema>>(
        searchParams,
        postFilterFormSchema,
      ),
    [searchParams],
  );
          ...
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.post.fields.user}</FormLabel>
