...
async function goToSystemAccountForm(page: Page) {
  ...
  const systemAccount = await prisma.systemAccount.findFirstOrThrow();

  // TODO: Create systemAccount here
  // await prisma.systemAccount.create({});

  await page.waitForURL('/');
  await page.goto(`/system-account/${systemAccount.id}/edit`);
  await page.waitForURL(`/system-account/${systemAccount.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToSystemAccountForm(page);
});
