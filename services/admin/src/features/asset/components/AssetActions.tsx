...
import { assetDestroyManyApiCall, assetArchiveManyApiCall, assetRestoreManyApiCall } from 'src/features/asset/assetApiCalls';
...
export function AssetActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.assetUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return assetDestroyManyApiCall([asset.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.asset.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      return assetArchiveManyApiCall([asset.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['asset'],
      ...
      toast({
        description: dictionary.asset.archive.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      return assetRestoreManyApiCall([asset.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['asset'],
      ...
      toast({
        description: dictionary.asset.restore.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/asset/${asset.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/asset/${asset.id}/edit`} prefetch={false}>
          ...
          {hasPermissionToArchive && !asset?.archivedAt && (
            <DropdownMenuItem
          ...
          {hasPermissionToRestore && Boolean(asset?.archivedAt) && (
            <DropdownMenuItem
      ...
      {archiveDialogOpen && (
        <ConfirmDialog
          title={dictionary.asset.archive.confirmTitle}
      ...
      {restoreDialogOpen && (
        <ConfirmDialog
          title={dictionary.asset.restore.confirmTitle}
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.asset.destroy.confirmTitle}
