...
async function goToArticleNew(page: Page) {
  ...
  await page.waitForURL(`/article/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToArticleNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.article.new.title }),
  ).toBeTruthy();
});
