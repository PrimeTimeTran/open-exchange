...
import { articleFilterFormSchema } from 'src/features/article/articleSchemas';
...
function ArticleListFilter({
  ...
  const previewRenders = {
    title: {
      label: dictionary.article.fields.title,
    },
    body: {
      label: dictionary.article.fields.body,
    },
    type: {
      label: dictionary.article.fields.type,
      render: dataTableFilterRenders(context).stringArray(),
    },
    user: {
      label: dictionary.article.fields.user,
  ...
  const filter = useMemo(
    () =>
      DataTableQueryParams.getFilter<z.infer<typeof articleFilterFormSchema>>(
        searchParams,
        articleFilterFormSchema,
      ),
    [searchParams],
  );

  const form = useForm({
    resolver: zodResolver(articleFilterFormSchema),
    mode: 'onSubmit',
  ...
  useEffect(() => {
    form.reset({ ...emptyValues, ...filter } as z.infer<
      typeof articleFilterFormSchema
    >);
  }, [filter, form]);
          ...
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.article.fields.title}</FormLabel>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.article.fields.body}</FormLabel>
                    <Input disabled={isLoading} {...field} />
                    <FormMessage />
                  </FormItem>
                )}
