...
async function goToLedgerEntryList(page: Page) {
  ...
  // TODO: Create ledgerEntry here
  // await prisma.ledgerEntry.create({});

  await page.waitForURL('/');
  await page.goto(`/ledger-entry`);
  await page.waitForURL(`/ledger-entry`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToLedgerEntryList(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.ledgerEntry.list.title }),
  ).toBeTruthy();
});
