...
import {
  ledgerEntryDestroyManyApiCall,
  ledgerEntryFindManyApiCall,
  ledgerEntryArchiveManyApiCall,
  ledgerEntryRestoreManyApiCall,
} from 'src/features/ledgerEntry/ledgerEntryApiCalls';
import { ledgerEntryExporterMapper } from 'src/features/ledgerEntry/ledgerEntryExporterMapper';
import { ledgerEntryFilterInputSchema } from 'src/features/ledgerEntry/ledgerEntrySchemas';
...
export default function LedgerEntryListActions({
  context,
  table,
  count,
  filter,
  ...
  filter: z.input<typeof ledgerEntryFilterInputSchema>;
  sorting: DataTableSort;
  count?: number;
  context: AppContext;
  table: Table<LedgerEntry>;
}) {
  ...
  const hasPermissionToCreate = hasPermission(permissions.ledgerEntryCreate, context);
  ...
  const hasPermissionToImport = hasPermission(permissions.ledgerEntryImport, context);

  const exportMutation = useMutation({
    mutationFn: () => {
      return ledgerEntryFindManyApiCall({
        filter: filter,
    ...
    onSuccess: (data) => {
      csvExporter(
        ledgerEntryExporterMapper(data.ledgerEntries, context),
        dictionary.ledgerEntry.fields,
        'ledgerEntries',
      );
      toast({
        description: dictionary.ledgerEntry.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.ledgerEntry.destroyMany.noSelection);
      }

      return ledgerEntryDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.ledgerEntry.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.ledgerEntry.archiveMany.noSelection);
      }

      return ledgerEntryArchiveManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.ledgerEntry.archiveMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.ledgerEntry.restoreMany.noSelection);
      }

      return ledgerEntryRestoreManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.ledgerEntry.restoreMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/ledger-entry/importer`} prefetch={false}>
      ...
      {archiveManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.ledgerEntry.archiveMany.confirmTitle}
          description={formatTranslation(
            dictionary.ledgerEntry.archiveMany.confirmDescription,
            selectedCount,
          )}
      ...
      {restoreManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.ledgerEntry.restoreMany.confirmTitle}
          description={formatTranslation(
            dictionary.ledgerEntry.restoreMany.confirmDescription,
            selectedCount,
          )}
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.ledgerEntry.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.ledgerEntry.destroyMany.confirmDescription,
            selectedCount,
          )}
