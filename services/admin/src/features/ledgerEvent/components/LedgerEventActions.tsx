...
import { ledgerEventDestroyManyApiCall, ledgerEventArchiveManyApiCall, ledgerEventRestoreManyApiCall } from 'src/features/ledgerEvent/ledgerEventApiCalls';
...
export function LedgerEventActions({
  mode,
  ledgerEvent,
  context,
}: {
  ...
  const hasPermissionToEdit = hasPermission(permissions.ledgerEventUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return ledgerEventDestroyManyApiCall([ledgerEvent.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.ledgerEvent.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      return ledgerEventArchiveManyApiCall([ledgerEvent.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['ledgerEvent'],
      ...
      if (mode === 'view') {
        router.push('/ledgerEvent');
      }

      toast({
        description: dictionary.ledgerEvent.archive.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      return ledgerEventRestoreManyApiCall([ledgerEvent.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['ledgerEvent'],
      ...
      toast({
        description: dictionary.ledgerEvent.restore.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/ledger-event/${ledgerEvent.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/ledger-event/${ledgerEvent.id}/edit`} prefetch={false}>
          ...
          {hasPermissionToArchive && !ledgerEvent?.archivedAt && (
            <DropdownMenuItem
          ...
          {hasPermissionToRestore && Boolean(ledgerEvent?.archivedAt) && (
            <DropdownMenuItem
      ...
      {archiveDialogOpen && (
        <ConfirmDialog
          title={dictionary.ledgerEvent.archive.confirmTitle}
      ...
      {restoreDialogOpen && (
        <ConfirmDialog
          title={dictionary.ledgerEvent.restore.confirmTitle}
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.ledgerEvent.destroy.confirmTitle}
