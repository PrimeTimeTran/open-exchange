...
import { ledgerEventUpdateController } from 'src/features/ledgerEvent/controllers/ledgerEventUpdateController';
...
async function createLedgerEvent(context: AppContext): Promise<any> {
...
describe('ledgerEventUpdate', () => {
  ...
  it.skip('must be signed in', async () => {
    const ledgerEvent = await createLedgerEvent(
      await testContext({
    ...
    try {
      await ledgerEventUpdateController(
        { id: ledgerEvent.id },
        {},
        await testContext(),
      );
  ...
  //     const previousLedgerEvent = await createLedgerEvent(
  //       await testContext({
  //         currentUserId: currentUser?.id,
  //         currentTenantId: currentTenant?.id,
  //       }),
  ...
  //     const updatedLedgerEvent = await prisma.ledgerEvent.findUniqueOrThrow({
  //       where: { id: previousLedgerEvent?.id },
  //       include: {},
  //     });
