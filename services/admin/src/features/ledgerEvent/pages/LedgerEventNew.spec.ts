...
async function goToLedgerEventNew(page: Page) {
  ...
  await page.goto(`/ledger-event/new`);
  await page.waitForURL(`/ledger-event/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToLedgerEventNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.ledgerEvent.new.title }),
  ).toBeTruthy();
});
