...
import {
  commentDestroyManyApiCall,
  commentFindManyApiCall,
  commentArchiveManyApiCall,
  commentRestoreManyApiCall,
} from 'src/features/comment/commentApiCalls';
import { commentExporterMapper } from 'src/features/comment/commentExporterMapper';
import { commentFilterInputSchema } from 'src/features/comment/commentSchemas';
...
export default function CommentListActions({
  context,
  table,
  count,
  filter,
  ...
  filter: z.input<typeof commentFilterInputSchema>;
  ...
  const hasPermissionToCreate = hasPermission(permissions.commentCreate, context);
  ...
  const hasPermissionToImport = hasPermission(permissions.commentImport, context);
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      csvExporter(
        commentExporterMapper(data.comments, context),
        dictionary.comment.fields,
        'comments',
      );
      toast({
        description: dictionary.comment.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.comment.destroyMany.noSelection);
      }

      return commentDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.comment.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.comment.archiveMany.noSelection);
      }

      return commentArchiveManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.comment.archiveMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.comment.restoreMany.noSelection);
      }

      return commentRestoreManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.comment.restoreMany.success,
      });
    },
    onError: (error: any) => {
      toast({
    ...
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/comment/importer`} prefetch={false}>
      ...
      {hasPermissionToCreate && (
        <Button
          ...
          asChild
        >
          <Link href={`/comment/new`} prefetch={false}>
      ...
      {archiveManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.comment.archiveMany.confirmTitle}
          description={formatTranslation(
            dictionary.comment.archiveMany.confirmDescription,
            selectedCount,
          )}
      ...
      {restoreManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.comment.restoreMany.confirmTitle}
          description={formatTranslation(
            dictionary.comment.restoreMany.confirmDescription,
            selectedCount,
          )}
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.comment.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.comment.destroyMany.confirmDescription,
            selectedCount,
          )}
