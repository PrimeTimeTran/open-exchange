...
import {
  withdrawalDestroyManyApiCall,
  withdrawalFindManyApiCall,
  withdrawalArchiveManyApiCall,
  withdrawalRestoreManyApiCall,
} from 'src/features/withdrawal/withdrawalApiCalls';
import { withdrawalExporterMapper } from 'src/features/withdrawal/withdrawalExporterMapper';
import { withdrawalFilterInputSchema } from 'src/features/withdrawal/withdrawalSchemas';
...
export default function WithdrawalListActions({
  context,
  table,
  count,
  filter,
  ...
  filter: z.input<typeof withdrawalFilterInputSchema>;
  ...
  const hasPermissionToCreate = hasPermission(permissions.withdrawalCreate, context);
  ...
  const hasPermissionToImport = hasPermission(permissions.withdrawalImport, context);
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      csvExporter(
        withdrawalExporterMapper(data.withdrawals, context),
        dictionary.withdrawal.fields,
        'withdrawals',
      );
      toast({
        description: dictionary.withdrawal.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.withdrawal.destroyMany.noSelection);
      }

      return withdrawalDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.withdrawal.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.withdrawal.archiveMany.noSelection);
      }

      return withdrawalArchiveManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.withdrawal.archiveMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.withdrawal.restoreMany.noSelection);
      }

      return withdrawalRestoreManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.withdrawal.restoreMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/withdrawal/importer`} prefetch={false}>
      ...
      {archiveManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.withdrawal.archiveMany.confirmTitle}
          description={formatTranslation(
            dictionary.withdrawal.archiveMany.confirmDescription,
            selectedCount,
          )}
      ...
      {restoreManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.withdrawal.restoreMany.confirmTitle}
          description={formatTranslation(
            dictionary.withdrawal.restoreMany.confirmDescription,
            selectedCount,
          )}
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.withdrawal.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.withdrawal.destroyMany.confirmDescription,
            selectedCount,
          )}
