...
async function goToWithdrawalList(page: Page) {
  ...
  await page.waitForURL(`/withdrawal`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToWithdrawalList(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.withdrawal.list.title }),
  ).toBeTruthy();
});
