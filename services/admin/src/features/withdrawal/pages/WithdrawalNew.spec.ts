...
async function goToWithdrawalNew(page: Page) {
  ...
  await page.goto(`/withdrawal/new`);
  await page.waitForURL(`/withdrawal/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToWithdrawalNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.withdrawal.new.title }),
  ).toBeTruthy();
});
