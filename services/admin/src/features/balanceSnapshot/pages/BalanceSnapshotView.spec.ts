...
async function goToBalanceSnapshotView(page: Page) {
  ...
  await page.goto(`/balance-snapshot/${balanceSnapshot.id}`);
  await page.waitForURL(`/balance-snapshot/${balanceSnapshot.id}`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToBalanceSnapshotView(page);
});
