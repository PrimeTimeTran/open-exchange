...
import { orderDestroyManyApiCall, orderArchiveManyApiCall, orderRestoreManyApiCall } from 'src/features/order/orderApiCalls';
...
export function OrderActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.orderUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return orderDestroyManyApiCall([order.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.order.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      return orderArchiveManyApiCall([order.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['order'],
      ...
      toast({
        description: dictionary.order.archive.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      return orderRestoreManyApiCall([order.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['order'],
      ...
      toast({
        description: dictionary.order.restore.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/order/${order.id}/edit`} prefetch={false}>
          ...
          {hasPermissionToArchive && !order?.archivedAt && (
            <DropdownMenuItem
          ...
          {hasPermissionToRestore && Boolean(order?.archivedAt) && (
            <DropdownMenuItem
      ...
      {archiveDialogOpen && (
        <ConfirmDialog
          title={dictionary.order.archive.confirmTitle}
      ...
      {restoreDialogOpen && (
        <ConfirmDialog
          title={dictionary.order.restore.confirmTitle}
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.order.destroy.confirmTitle}
