...
import { chateeFilterFormSchema } from 'src/features/chatee/chateeSchemas';
...
function ChateeListFilter({
  ...
  const previewRenders = {
    nickname: {
      label: dictionary.chatee.fields.nickname,
    },
    status: {
      label: dictionary.chatee.fields.status,
      render: dataTableFilterRenders(context).enumerator(
        dictionary.chatee.enumerators.status,
      ),
    },
    role: {
      label: dictionary.chatee.fields.role,
  ...
  const filter = useMemo(
    () =>
      DataTableQueryParams.getFilter<z.infer<typeof chateeFilterFormSchema>>(
        searchParams,
        chateeFilterFormSchema,
      ),
    [searchParams],
  );

  const form = useForm({
    resolver: zodResolver(chateeFilterFormSchema),
    mode: 'onSubmit',
          ...
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.chatee.fields.nickname}</FormLabel>
...
export default ChateeListFilter;
