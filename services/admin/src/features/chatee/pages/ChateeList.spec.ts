...
async function goToChateeList(page: Page) {
  ...
  await page.goto(`/chatee`);
  await page.waitForURL(`/chatee`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToChateeList(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.chatee.list.title }),
  ).toBeTruthy();
});
