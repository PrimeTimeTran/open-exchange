...
import { chateeUpdateController } from 'src/features/chatee/controllers/chateeUpdateController';
...
async function createChatee(context: AppContext): Promise<any> {
  const prisma = prismaAuth(context);
  const currentTenant = await prisma.tenant.findFirstOrThrow();
  // TODO: Implement your own logic here
  // await prisma.chatee.create({});
}

describe('chateeUpdate', () => {
  ...
  it.skip('must be signed in', async () => {
    const chatee = await createChatee(
      await testContext({
  ...
  //     const previousChatee = await createChatee(
  //       await testContext({
  //         currentUserId: currentUser?.id,
  //         currentTenantId: currentTenant?.id,
  //       }),
  ...
  //     const updatedChatee = await prisma.chatee.findUniqueOrThrow({
  //       where: { id: previousChatee?.id },
  //       include: {},
  //     });
