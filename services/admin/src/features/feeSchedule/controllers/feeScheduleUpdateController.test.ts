...
import { feeScheduleUpdateController } from 'src/features/feeSchedule/controllers/feeScheduleUpdateController';
...
async function createFeeSchedule(context: AppContext): Promise<any> {
...
describe('feeScheduleUpdate', () => {
  ...
  it.skip('must be signed in', async () => {
    const feeSchedule = await createFeeSchedule(
      await testContext({
    ...
    try {
      await feeScheduleUpdateController(
        { id: feeSchedule.id },
        {},
        await testContext(),
      );
  ...
  //     const previousFeeSchedule = await createFeeSchedule(
  //       await testContext({
  //         currentUserId: currentUser?.id,
  //         currentTenantId: currentTenant?.id,
  //       }),
  ...
  //     const updatedFeeSchedule = await prisma.feeSchedule.findUniqueOrThrow({
  //       where: { id: previousFeeSchedule?.id },
  //       include: {},
  //     });
