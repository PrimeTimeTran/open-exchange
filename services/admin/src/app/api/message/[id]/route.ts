...
import { messageUpdateController } from 'src/features/message/controllers/messageUpdateController';
...
export async function GET(
  ...
  try {
    ...
    const payload = await messageFindController(params, context);
...
export async function PUT(
  ...
  try {
    ...
    const payload = await messageUpdateController(params, body, context);
    return NextResponseSuccess(request, context, payload);
  } catch (error: any) {
    return NextResponseError(request, context, error);
  }
