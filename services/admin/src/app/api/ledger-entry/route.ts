...
import { ledgerEntryCreateController } from 'src/features/ledgerEntry/controllers/ledgerEntryCreateController';
import { ledgerEntryDestroyManyController } from 'src/features/ledgerEntry/controllers/ledgerEntryDestroyManyController';
import { ledgerEntryFindManyController } from 'src/features/ledgerEntry/controllers/ledgerEntryFindManyController';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const payload = await ledgerEntryCreateController(body, context);
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const payload = await ledgerEntryFindManyController(query, context);
    return NextResponseSuccess(request, context, payload);
  } catch (error: any) {
    return NextResponseError(request, context, error);
  }
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    context = await appContext(request);
    const query = queryToObject(request.nextUrl.search);
    await ledgerEntryDestroyManyController(query, context);
    const payload = true;
    return NextResponseSuccess(request, context, payload);
  } catch (error: any) {
    return NextResponseError(request, context, error);
