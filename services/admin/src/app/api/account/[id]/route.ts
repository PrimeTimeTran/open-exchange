...
import { accountUpdateController } from 'src/features/account/controllers/accountUpdateController';
...
export async function GET(
  ...
  try {
    ...
    const payload = await accountFindController(params, context);
...
export async function PUT(
  ...
  try {
    ...
    const payload = await accountUpdateController(params, body, context);
    return NextResponseSuccess(request, context, payload);
  } catch (error: any) {
    return NextResponseError(request, context, error);
  }
