...
import { feedbackDestroyManyApiCall, feedbackArchiveManyApiCall, feedbackRestoreManyApiCall } from 'src/features/feedback/feedbackApiCalls';
...
export function FeedbackActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.feedbackUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return feedbackDestroyManyApiCall([feedback.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.feedback.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      return feedbackArchiveManyApiCall([feedback.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['feedback'],
      ...
      toast({
        description: dictionary.feedback.archive.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      return feedbackRestoreManyApiCall([feedback.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['feedback'],
      ...
      toast({
        description: dictionary.feedback.restore.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/feedback/${feedback.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/feedback/${feedback.id}/edit`} prefetch={false}>
          ...
          {hasPermissionToArchive && !feedback?.archivedAt && (
            <DropdownMenuItem
          ...
          {hasPermissionToRestore && Boolean(feedback?.archivedAt) && (
            <DropdownMenuItem
      ...
      {archiveDialogOpen && (
        <ConfirmDialog
          title={dictionary.feedback.archive.confirmTitle}
      ...
      {restoreDialogOpen && (
        <ConfirmDialog
          title={dictionary.feedback.restore.confirmTitle}
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.feedback.destroy.confirmTitle}
