...
import { feedbackFilterFormSchema } from 'src/features/feedback/feedbackSchemas';
...
function FeedbackListFilter({
  ...
  const previewRenders = {
    title: {
      label: dictionary.feedback.fields.title,
    },
    description: {
      label: dictionary.feedback.fields.description,
    },
    type: {
      label: dictionary.feedback.fields.type,
      render: dataTableFilterRenders(context).enumerator(
        dictionary.feedback.enumerators.type,
      ),
    },
    status: {
      label: dictionary.feedback.fields.status,
      render: dataTableFilterRenders(context).enumerator(
  ...
  const filter = useMemo(
    () =>
      DataTableQueryParams.getFilter<z.infer<typeof feedbackFilterFormSchema>>(
        searchParams,
  ...
  const form = useForm({
    resolver: zodResolver(feedbackFilterFormSchema),
    mode: 'onSubmit',
  ...
  useEffect(() => {
    form.reset({ ...emptyValues, ...filter } as z.infer<
      typeof feedbackFilterFormSchema
    >);
  }, [filter, form]);
          ...
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.feedback.fields.title}</FormLabel>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.feedback.fields.description}</FormLabel>
                    <Input disabled={isLoading} {...field} />
                    <FormMessage />
                  </FormItem>
                )}
