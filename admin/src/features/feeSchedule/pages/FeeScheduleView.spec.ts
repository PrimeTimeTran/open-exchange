...
async function goToFeeScheduleView(page: Page) {
  ...
  await page.goto(`/fee-schedule/${feeSchedule.id}`);
  await page.waitForURL(`/fee-schedule/${feeSchedule.id}`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToFeeScheduleView(page);
});
