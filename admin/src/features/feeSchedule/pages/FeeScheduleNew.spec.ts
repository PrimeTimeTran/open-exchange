...
async function goToFeeScheduleNew(page: Page) {
  ...
  await page.goto(`/fee-schedule/new`);
  await page.waitForURL(`/fee-schedule/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToFeeScheduleNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.feeSchedule.new.title }),
  ).toBeTruthy();
});
