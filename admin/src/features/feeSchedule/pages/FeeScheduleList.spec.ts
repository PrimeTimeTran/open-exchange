...
async function goToFeeScheduleList(page: Page) {
  ...
  // TODO: Create feeSchedule here
  // await prisma.feeSchedule.create({});

  await page.waitForURL('/');
  await page.goto(`/fee-schedule`);
  await page.waitForURL(`/fee-schedule`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToFeeScheduleList(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.feeSchedule.list.title }),
  ).toBeTruthy();
});
