...
import { fillFilterFormSchema } from 'src/features/fill/fillSchemas';
...
function FillListFilter({
  ...
  const previewRenders = {
    side: {
      label: dictionary.fill.fields.side,
      render: dataTableFilterRenders(context).enumerator(
        dictionary.fill.enumerators.side,
      ),
    },
    priceRange: {
      label: dictionary.fill.fields.price,
      render: dataTableFilterRenders(context).decimalRange(),
    },
    quantityRange: {
      label: dictionary.fill.fields.quantity,
      render: dataTableFilterRenders(context).decimalRange(),
    },
    feeRange: {
      label: dictionary.fill.fields.fee,
      render: dataTableFilterRenders(context).decimalRange(),
    },
    archived: {
  ...
  const filter = useMemo(
    () =>
      DataTableQueryParams.getFilter<z.infer<typeof fillFilterFormSchema>>(
        searchParams,
        fillFilterFormSchema,
      ),
    [searchParams],
  );

  const form = useForm({
    resolver: zodResolver(fillFilterFormSchema),
    mode: 'onSubmit',
          ...
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.fill.fields.price}</FormLabel>
                    <RangeInput
                      type="text"
                      dictionary={dictionary}
                      disabled={isLoading}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.fill.fields.quantity}</FormLabel>
                    <RangeInput
                      type="text"
                      dictionary={dictionary}
                      disabled={isLoading}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.fill.fields.fee}</FormLabel>
                    <RangeInput
                      type="text"
                      dictionary={dictionary}
                      disabled={isLoading}
