...
import { fillDestroyManyApiCall, fillArchiveManyApiCall, fillRestoreManyApiCall } from 'src/features/fill/fillApiCalls';
...
export function FillActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.fillUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return fillDestroyManyApiCall([fill.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.fill.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      return fillArchiveManyApiCall([fill.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['fill'],
      ...
      toast({
        description: dictionary.fill.archive.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      return fillRestoreManyApiCall([fill.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['fill'],
      ...
      toast({
        description: dictionary.fill.restore.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/fill/${fill.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/fill/${fill.id}/edit`} prefetch={false}>
          ...
          {hasPermissionToArchive && !fill?.archivedAt && (
            <DropdownMenuItem
          ...
          {hasPermissionToRestore && Boolean(fill?.archivedAt) && (
            <DropdownMenuItem
      ...
      {archiveDialogOpen && (
        <ConfirmDialog
          title={dictionary.fill.archive.confirmTitle}
      ...
      {restoreDialogOpen && (
        <ConfirmDialog
          title={dictionary.fill.restore.confirmTitle}
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.fill.destroy.confirmTitle}
