...
async function goToFillNew(page: Page) {
  ...
  await page.goto(`/fill/new`);
  await page.waitForURL(`/fill/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToFillNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.fill.new.title }),
  ).toBeTruthy();
});
