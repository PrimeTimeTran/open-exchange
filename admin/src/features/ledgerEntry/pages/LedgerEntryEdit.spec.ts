...
async function goToLedgerEntryForm(page: Page) {
  ...
  await page.goto(`/ledger-entry/${ledgerEntry.id}/edit`);
  await page.waitForURL(`/ledger-entry/${ledgerEntry.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToLedgerEntryForm(page);
});
