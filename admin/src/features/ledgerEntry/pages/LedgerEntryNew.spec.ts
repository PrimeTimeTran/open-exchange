...
async function goToLedgerEntryNew(page: Page) {
  ...
  await page.goto(`/ledger-entry/new`);
  await page.waitForURL(`/ledger-entry/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToLedgerEntryNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.ledgerEntry.new.title }),
  ).toBeTruthy();
});
