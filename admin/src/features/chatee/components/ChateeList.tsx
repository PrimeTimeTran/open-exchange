...
import ChateeListActions from 'src/features/chatee/components/ChateeListActions';
import ChateeListFilter from 'src/features/chatee/components/ChateeListFilter';
import { chateeFindManyApiCall } from 'src/features/chatee/chateeApiCalls';
import {
  ChateeWithRelationships,
  chateeFilterInputSchema,
...
import { ChateeNewButton } from 'src/features/chatee/components/ChateeNewButton';
import { z } from 'zod';
...
export default function ChateeList({ context }: { context: AppContext }) {
  ...
  const filter = useMemo(() => {
    return DataTableQueryParams.getFilter<
      z.input<typeof chateeFilterInputSchema>
    >(searchParams, chateeFilterInputSchema);
  }, [searchParams]);

  const columns: ColumnDef<ChateeWithRelationships>[] = [
    {
    ...
    },
    {
      ...
      meta: {
        title: dictionary.chatee.fields.nickname,
      },
    },
    {
      accessorKey: 'status',
      meta: {
        title: dictionary.chatee.fields.status,
      },
      cell: ({ row }) => {
    ...
    },
    {
      accessorKey: 'role',
      meta: {
        title: dictionary.chatee.fields.role,
      },
      enableSorting: false,
      cell: ({ row }) => {
        return (
  ...
  const query = useQuery({
    queryKey: ['chatee', 'list', filter, sorting, pagination],
  ...
  const table = useReactTable({
    ...
    data: query.data?.chatees || defaultData,
    columns,
  ...
  return (
    <div className="mb-4 flex w-full flex-col gap-4">
      <div className="flex items-center justify-between">
        <Breadcrumb items={[[dictionary.chatee.list.menu]]} />
      ...
      <ChateeListFilter context={context} isLoading={query.isLoading} />
      ...
      <DataTable
        ...
        notFoundText={dictionary.chatee.list.noResults}
        newButton={<ChateeNewButton context={context} />}
      />
