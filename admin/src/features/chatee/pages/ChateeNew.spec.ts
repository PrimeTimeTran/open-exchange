...
async function goToChateeNew(page: Page) {
  ...
  await page.goto(`/chatee/new`);
  await page.waitForURL(`/chatee/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToChateeNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.chatee.new.title }),
  ).toBeTruthy();
});
