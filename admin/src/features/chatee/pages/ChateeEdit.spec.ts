...
async function goToChateeForm(page: Page) {
  ...
  const chatee = await prisma.chatee.findFirstOrThrow();

  // TODO: Create chatee here
  // await prisma.chatee.create({});

  await page.waitForURL('/');
  await page.goto(`/chatee/${chatee.id}/edit`);
  await page.waitForURL(`/chatee/${chatee.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToChateeForm(page);
});
