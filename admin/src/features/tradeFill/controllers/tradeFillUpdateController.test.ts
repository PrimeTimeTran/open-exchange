...
import { tradeFillUpdateController } from 'src/features/tradeFill/controllers/tradeFillUpdateController';
...
describe('tradeFillUpdate', () => {
  ...
  it.skip('must be signed in', async () => {
    const tradeFill = await createTradeFill(
      await testContext({
    ...
    try {
      await tradeFillUpdateController(
        { id: tradeFill.id },
        {},
        await testContext(),
      );
  ...
  //     const previousTradeFill = await createTradeFill(
  //       await testContext({
  //         currentUserId: currentUser?.id,
  //         currentTenantId: currentTenant?.id,
  //       }),
  ...
  //     const updatedTradeFill = await prisma.tradeFill.findUniqueOrThrow({
  //       where: { id: previousTradeFill?.id },
  //       include: {},
  //     });
