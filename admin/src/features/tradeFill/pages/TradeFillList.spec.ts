...
async function goToTradeFillList(page: Page) {
  ...
  await page.goto(`/trade-fill`);
  await page.waitForURL(`/trade-fill`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToTradeFillList(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.tradeFill.list.title }),
  ).toBeTruthy();
});
