...
async function goToTradeFillNew(page: Page) {
  ...
  await page.goto(`/trade-fill/new`);
  await page.waitForURL(`/trade-fill/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToTradeFillNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.tradeFill.new.title }),
  ).toBeTruthy();
});
