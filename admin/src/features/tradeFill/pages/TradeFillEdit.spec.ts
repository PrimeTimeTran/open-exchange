...
async function goToTradeFillForm(page: Page) {
  ...
  const tradeFill = await prisma.tradeFill.findFirstOrThrow();

  // TODO: Create tradeFill here
  // await prisma.tradeFill.create({});

  await page.waitForURL('/');
  await page.goto(`/trade-fill/${tradeFill.id}/edit`);
  await page.waitForURL(`/trade-fill/${tradeFill.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToTradeFillForm(page);
});
