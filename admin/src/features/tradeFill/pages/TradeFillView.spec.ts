...
async function goToTradeFillView(page: Page) {
  ...
  await page.goto(`/trade-fill/${tradeFill.id}`);
  await page.waitForURL(`/trade-fill/${tradeFill.id}`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToTradeFillView(page);
});
