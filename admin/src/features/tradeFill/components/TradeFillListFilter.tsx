...
import { tradeFillFilterFormSchema } from 'src/features/tradeFill/tradeFillSchemas';
...
function TradeFillListFilter({
  ...
  const previewRenders = {
    side: {
      label: dictionary.tradeFill.fields.side,
      render: dataTableFilterRenders(context).enumerator(
        dictionary.tradeFill.enumerators.side,
      ),
    },
    priceRange: {
      label: dictionary.tradeFill.fields.price,
      render: dataTableFilterRenders(context).decimalRange(),
    },
    quantityRange: {
      label: dictionary.tradeFill.fields.quantity,
      render: dataTableFilterRenders(context).decimalRange(),
    },
    feeRange: {
      label: dictionary.tradeFill.fields.fee,
      render: dataTableFilterRenders(context).decimalRange(),
    },
    archived: {
  ...
  const filter = useMemo(
    () =>
      DataTableQueryParams.getFilter<z.infer<typeof tradeFillFilterFormSchema>>(
        searchParams,
        tradeFillFilterFormSchema,
      ),
    [searchParams],
  );

  const form = useForm({
    resolver: zodResolver(tradeFillFilterFormSchema),
    mode: 'onSubmit',
  ...
  useEffect(() => {
    form.reset({ ...emptyValues, ...filter } as z.infer<
      typeof tradeFillFilterFormSchema
    >);
  }, [filter, form]);
          ...
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.tradeFill.fields.price}</FormLabel>
                    <RangeInput
                      type="text"
                      dictionary={dictionary}
                      disabled={isLoading}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.tradeFill.fields.quantity}</FormLabel>
                    <RangeInput
                      type="text"
                      dictionary={dictionary}
                      disabled={isLoading}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.tradeFill.fields.fee}</FormLabel>
                    <RangeInput
                      type="text"
                      dictionary={dictionary}
                      disabled={isLoading}
...
export default TradeFillListFilter;
