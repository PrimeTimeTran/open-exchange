...
import {
  tradeFillDestroyManyApiCall,
  tradeFillFindManyApiCall,
  tradeFillArchiveManyApiCall,
  tradeFillRestoreManyApiCall,
} from 'src/features/tradeFill/tradeFillApiCalls';
import { tradeFillExporterMapper } from 'src/features/tradeFill/tradeFillExporterMapper';
import { tradeFillFilterInputSchema } from 'src/features/tradeFill/tradeFillSchemas';
...
export default function TradeFillListActions({
  context,
  table,
  count,
  filter,
  ...
  filter: z.input<typeof tradeFillFilterInputSchema>;
  sorting: DataTableSort;
  count?: number;
  context: AppContext;
  table: Table<TradeFill>;
}) {
  ...
  const hasPermissionToCreate = hasPermission(permissions.tradeFillCreate, context);
  ...
  const hasPermissionToImport = hasPermission(permissions.tradeFillImport, context);
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      csvExporter(
        tradeFillExporterMapper(data.tradeFills, context),
        dictionary.tradeFill.fields,
        'tradeFills',
      );
      toast({
        description: dictionary.tradeFill.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.tradeFill.destroyMany.noSelection);
      }

      return tradeFillDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.tradeFill.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.tradeFill.archiveMany.noSelection);
      }

      return tradeFillArchiveManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.tradeFill.archiveMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.tradeFill.restoreMany.noSelection);
      }

      return tradeFillRestoreManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.tradeFill.restoreMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/trade-fill/importer`} prefetch={false}>
      ...
      {archiveManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.tradeFill.archiveMany.confirmTitle}
          description={formatTranslation(
            dictionary.tradeFill.archiveMany.confirmDescription,
            selectedCount,
          )}
      ...
      {restoreManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.tradeFill.restoreMany.confirmTitle}
          description={formatTranslation(
            dictionary.tradeFill.restoreMany.confirmDescription,
            selectedCount,
          )}
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.tradeFill.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.tradeFill.destroyMany.confirmDescription,
            selectedCount,
          )}
