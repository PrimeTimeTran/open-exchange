...
async function goToMessageForm(page: Page) {
  ...
  await page.goto(`/message/${message.id}/edit`);
  await page.waitForURL(`/message/${message.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToMessageForm(page);
});
