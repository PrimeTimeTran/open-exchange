...
async function goToMessageNew(page: Page) {
  ...
  await page.goto(`/message/new`);
  await page.waitForURL(`/message/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToMessageNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.message.new.title }),
  ).toBeTruthy();
});
