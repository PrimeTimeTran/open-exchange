...
import { messageDestroyManyApiCall, messageArchiveManyApiCall, messageRestoreManyApiCall } from 'src/features/message/messageApiCalls';
...
export function MessageActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.messageUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return messageDestroyManyApiCall([message.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.message.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      return messageArchiveManyApiCall([message.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['message'],
      ...
      toast({
        description: dictionary.message.archive.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      return messageRestoreManyApiCall([message.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['message'],
      ...
      toast({
        description: dictionary.message.restore.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/message/${message.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/message/${message.id}/edit`} prefetch={false}>
          ...
          {hasPermissionToArchive && !message?.archivedAt && (
            <DropdownMenuItem
          ...
          {hasPermissionToRestore && Boolean(message?.archivedAt) && (
            <DropdownMenuItem
      ...
      {archiveDialogOpen && (
        <ConfirmDialog
          title={dictionary.message.archive.confirmTitle}
      ...
      {restoreDialogOpen && (
        <ConfirmDialog
          title={dictionary.message.restore.confirmTitle}
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.message.destroy.confirmTitle}
