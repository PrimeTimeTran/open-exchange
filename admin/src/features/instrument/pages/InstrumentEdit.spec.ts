...
async function goToInstrumentForm(page: Page) {
  ...
  // TODO: Create instrument here
  // await prisma.instrument.create({});

  await page.waitForURL('/');
  await page.goto(`/instrument/${instrument.id}/edit`);
  await page.waitForURL(`/instrument/${instrument.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToInstrumentForm(page);
});
