...
import { instrumentImporterController } from 'src/features/instrument/controllers/instrumentImporterController';
...
async function buildInstrument(context: AppContext) {
...
describe('instrumentImport', () => {
  ...
  it('must be signed in', async () => {
    const body = await buildInstrument(
      await testContext({
    ...
    try {
      await instrumentImporterController(body, await testContext());
      fail();
    } catch (error: any) {
  ...
  it('must have permission', async () => {
    ...
    try {
      await instrumentImporterController(
        body,
        await testContext({
          currentUserId: currentUser?.id,
          currentTenantId: currentTenant?.id,
  ...
  it.skip('imports', async () => {
    ...
    const response = await instrumentImporterController(
