...
import { accountFilterFormSchema } from 'src/features/account/accountSchemas';
...
function AccountListFilter({
  ...
  const previewRenders = {
    ...
    status: {
      label: dictionary.account.fields.status,
      render: dataTableFilterRenders(context).enumerator(
  ...
  const filter = useMemo(
    () =>
      DataTableQueryParams.getFilter<z.infer<typeof accountFilterFormSchema>>(
        searchParams,
        accountFilterFormSchema,
      ),
    [searchParams],
  );

  const form = useForm({
    resolver: zodResolver(accountFilterFormSchema),
    mode: 'onSubmit',
  ...
  useEffect(() => {
    form.reset({ ...emptyValues, ...filter } as z.infer<
      typeof accountFilterFormSchema
    >);
  }, [filter, form]);
