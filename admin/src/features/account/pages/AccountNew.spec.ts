...
async function goToAccountNew(page: Page) {
  ...
  await page.waitForURL(`/account/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToAccountNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.account.new.title }),
  ).toBeTruthy();
});
