...
import {
  walletDestroyManyApiCall,
  walletFindManyApiCall,
  walletArchiveManyApiCall,
  walletRestoreManyApiCall,
} from 'src/features/wallet/walletApiCalls';
import { walletExporterMapper } from 'src/features/wallet/walletExporterMapper';
import { walletFilterInputSchema } from 'src/features/wallet/walletSchemas';
...
export default function WalletListActions({
  context,
  table,
  count,
  filter,
  ...
  filter: z.input<typeof walletFilterInputSchema>;
  ...
  const hasPermissionToCreate = hasPermission(permissions.walletCreate, context);
  ...
  const hasPermissionToImport = hasPermission(permissions.walletImport, context);

  const exportMutation = useMutation({
    mutationFn: () => {
      return walletFindManyApiCall({
        filter: filter,
    ...
    onSuccess: (data) => {
      csvExporter(
        walletExporterMapper(data.wallets, context),
        dictionary.wallet.fields,
        'wallets',
      );
      toast({
        description: dictionary.wallet.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.wallet.destroyMany.noSelection);
      }

      return walletDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.wallet.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.wallet.archiveMany.noSelection);
      }

      return walletArchiveManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.wallet.archiveMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.wallet.restoreMany.noSelection);
      }

      return walletRestoreManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.wallet.restoreMany.success,
      });
    },
    onError: (error: any) => {
      toast({
    ...
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/wallet/importer`} prefetch={false}>
      ...
      {hasPermissionToCreate && (
        <Button
          ...
          asChild
        >
          <Link href={`/wallet/new`} prefetch={false}>
      ...
      {archiveManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.wallet.archiveMany.confirmTitle}
          description={formatTranslation(
            dictionary.wallet.archiveMany.confirmDescription,
            selectedCount,
          )}
      ...
      {restoreManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.wallet.restoreMany.confirmTitle}
          description={formatTranslation(
            dictionary.wallet.restoreMany.confirmDescription,
            selectedCount,
          )}
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.wallet.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.wallet.destroyMany.confirmDescription,
            selectedCount,
          )}
