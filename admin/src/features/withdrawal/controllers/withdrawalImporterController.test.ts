...
import { withdrawalImporterController } from 'src/features/withdrawal/controllers/withdrawalImporterController';
...
async function buildWithdrawal(context: AppContext) {
...
describe('withdrawalImport', () => {
  ...
  it('must be signed in', async () => {
    const body = await buildWithdrawal(
      await testContext({
    ...
    try {
      await withdrawalImporterController(body, await testContext());
      fail();
    } catch (error: any) {
  ...
  it('must have permission', async () => {
    ...
    try {
      await withdrawalImporterController(
        body,
        await testContext({
          currentUserId: currentUser?.id,
          currentTenantId: currentTenant?.id,
  ...
  it.skip('imports', async () => {
    ...
    const response = await withdrawalImporterController(
