...
async function goToLedgerEventView(page: Page) {
  ...
  await page.goto(`/ledger-event/${ledgerEvent.id}`);
  await page.waitForURL(`/ledger-event/${ledgerEvent.id}`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToLedgerEventView(page);
});
