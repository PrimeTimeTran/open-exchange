...
export function LedgerEventAutocompleteInput({
  onChange,
  value,
  selectPlaceholder,
  searchPlaceholder,
  ...
  onChange: (value: Partial<LedgerEvent> | undefined | null) => void;
  value?: Partial<LedgerEvent> | null;
      ...
      {isFormOpen && (
        <LedgerEventFormSheet
          ...
          onSuccess={(ledgerEvent) => {
            setIsFormOpen(false);
            onChange(ledgerEvent);
          }}
          context={context}
          ledgerEvent={value ? value : undefined}
        />
      )}
    </div>
  );
