...
import { systemAccountImporterController } from 'src/features/systemAccount/controllers/systemAccountImporterController';
...
async function buildSystemAccount(context: AppContext) {
...
describe('systemAccountImport', () => {
  ...
  it('must be signed in', async () => {
    const body = await buildSystemAccount(
      await testContext({
    ...
    try {
      await systemAccountImporterController(body, await testContext());
      fail();
    } catch (error: any) {
  ...
  it('must have permission', async () => {
    ...
    try {
      await systemAccountImporterController(
        body,
        await testContext({
          currentUserId: currentUser?.id,
          currentTenantId: currentTenant?.id,
  ...
  it.skip('imports', async () => {
    ...
    const response = await systemAccountImporterController(
