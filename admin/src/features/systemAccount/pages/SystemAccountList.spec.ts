...
async function goToSystemAccountList(page: Page) {
  ...
  await page.waitForURL(`/system-account`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToSystemAccountList(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.systemAccount.list.title }),
  ).toBeTruthy();
});
