...
async function goToSystemAccountNew(page: Page) {
  ...
  await page.goto(`/system-account/new`);
  await page.waitForURL(`/system-account/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToSystemAccountNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.systemAccount.new.title }),
  ).toBeTruthy();
});
