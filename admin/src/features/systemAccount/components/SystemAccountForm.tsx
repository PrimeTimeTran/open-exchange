...
import {
  systemAccountCreateApiCall,
  systemAccountUpdateApiCall,
...
export function SystemAccountForm({
  ...
  const isEditing = Boolean(systemAccount?.id);

  const [initialValues] = React.useState({
    type: systemAccount?.type || null,
    name: systemAccount?.name || '',
    description: systemAccount?.description || '',
    isActive: systemAccount?.isActive || false,
    meta: systemAccount?.meta?.toString() || '',
  });

  const form = useForm({
    resolver: zodResolver(systemAccountCreateInputSchema),
    mode: 'onSubmit',
  ...
  const mutation = useMutation({
    mutationFn: (data: z.input<typeof systemAccountCreateInputSchema>) => {
      if (systemAccount?.id) {
        return systemAccountUpdateApiCall(systemAccount.id, data);
      } else {
        return systemAccountCreateApiCall(data);
      }
    },
    onSuccess: (systemAccount: SystemAccount) => {
      ...
      toast({
        description: isEditing
          ? dictionary.systemAccount.edit.success
          : dictionary.systemAccount.new.success,
      });
    },
    onError: (error: Error) => {
      toast({
      ...
      >
        <div className="grid w-full gap-8">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{dictionary.systemAccount.fields.type}</FormLabel>

                <SelectInput
                  options={Object.keys(systemAccountEnumerators.type).map(
                    (value) => ({
                      value,
                ...
                {dictionary.systemAccount.hints.type ? (
                  <FormDescription>
                    {dictionary.systemAccount.hints.type}
                  </FormDescription>
                ) : null}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  {dictionary.systemAccount.hints.name ? (
                    <FormDescription>
                      {dictionary.systemAccount.hints.name}
                    </FormDescription>
                  ) : null}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  {dictionary.systemAccount.hints.description ? (
                    <FormDescription>
                      {dictionary.systemAccount.hints.description}
                    </FormDescription>
                  ) : null}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  {dictionary.systemAccount.hints.isActive ? (
                    <FormDescription>
                      {dictionary.systemAccount.hints.isActive}
                    </FormDescription>
                  ) : null}
          ...
          <div className="grid max-w-lg gap-1">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>
                    {dictionary.systemAccount.fields.meta}
                  </FormLabel>
                  ...
                  {dictionary.systemAccount.hints.meta ? (
                    <FormDescription>
                      {dictionary.systemAccount.hints.meta}
                    </FormDescription>
                  ) : null}
