...
import { systemAccountDestroyManyApiCall, systemAccountArchiveManyApiCall, systemAccountRestoreManyApiCall } from 'src/features/systemAccount/systemAccountApiCalls';
...
export function SystemAccountActions({
  mode,
  systemAccount,
  context,
}: {
  ...
  const hasPermissionToEdit = hasPermission(permissions.systemAccountUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return systemAccountDestroyManyApiCall([systemAccount.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.systemAccount.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      return systemAccountArchiveManyApiCall([systemAccount.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['systemAccount'],
      ...
      if (mode === 'view') {
        router.push('/systemAccount');
      }

      toast({
        description: dictionary.systemAccount.archive.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      return systemAccountRestoreManyApiCall([systemAccount.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['systemAccount'],
      ...
      toast({
        description: dictionary.systemAccount.restore.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/system-account/${systemAccount.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/system-account/${systemAccount.id}/edit`} prefetch={false}>
          ...
          {hasPermissionToArchive && !systemAccount?.archivedAt && (
            <DropdownMenuItem
          ...
          {hasPermissionToRestore && Boolean(systemAccount?.archivedAt) && (
            <DropdownMenuItem
      ...
      {archiveDialogOpen && (
        <ConfirmDialog
          title={dictionary.systemAccount.archive.confirmTitle}
      ...
      {restoreDialogOpen && (
        <ConfirmDialog
          title={dictionary.systemAccount.restore.confirmTitle}
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.systemAccount.destroy.confirmTitle}
