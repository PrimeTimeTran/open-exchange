...
async function goToArticleForm(page: Page) {
  ...
  // TODO: Create article here
  // await prisma.article.create({});

  await page.waitForURL('/');
  await page.goto(`/article/${article.id}/edit`);
  await page.waitForURL(`/article/${article.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToArticleForm(page);
});
