...
import { articleDestroyManyApiCall, articleArchiveManyApiCall, articleRestoreManyApiCall } from 'src/features/article/articleApiCalls';
...
export function ArticleActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.articleUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return articleDestroyManyApiCall([article.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.article.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      return articleArchiveManyApiCall([article.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['article'],
      ...
      toast({
        description: dictionary.article.archive.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      return articleRestoreManyApiCall([article.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['article'],
      ...
      toast({
        description: dictionary.article.restore.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/article/${article.id}/edit`} prefetch={false}>
          ...
          {hasPermissionToArchive && !article?.archivedAt && (
            <DropdownMenuItem
          ...
          {hasPermissionToRestore && Boolean(article?.archivedAt) && (
            <DropdownMenuItem
      ...
      {archiveDialogOpen && (
        <ConfirmDialog
          title={dictionary.article.archive.confirmTitle}
      ...
      {restoreDialogOpen && (
        <ConfirmDialog
          title={dictionary.article.restore.confirmTitle}
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.article.destroy.confirmTitle}
