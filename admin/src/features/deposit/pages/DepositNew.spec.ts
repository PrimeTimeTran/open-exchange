...
async function goToDepositNew(page: Page) {
  ...
  await page.waitForURL(`/deposit/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToDepositNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.deposit.new.title }),
  ).toBeTruthy();
});
