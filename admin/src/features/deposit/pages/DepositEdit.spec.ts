...
async function goToDepositForm(page: Page) {
  ...
  // TODO: Create deposit here
  // await prisma.deposit.create({});

  await page.waitForURL('/');
  await page.goto(`/deposit/${deposit.id}/edit`);
  await page.waitForURL(`/deposit/${deposit.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToDepositForm(page);
});
