...
import { depositDestroyManyApiCall, depositArchiveManyApiCall, depositRestoreManyApiCall } from 'src/features/deposit/depositApiCalls';
...
export function DepositActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.depositUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return depositDestroyManyApiCall([deposit.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.deposit.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      return depositArchiveManyApiCall([deposit.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['deposit'],
      ...
      toast({
        description: dictionary.deposit.archive.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      return depositRestoreManyApiCall([deposit.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['deposit'],
      ...
      toast({
        description: dictionary.deposit.restore.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/deposit/${deposit.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/deposit/${deposit.id}/edit`} prefetch={false}>
          ...
          {hasPermissionToArchive && !deposit?.archivedAt && (
            <DropdownMenuItem
          ...
          {hasPermissionToRestore && Boolean(deposit?.archivedAt) && (
            <DropdownMenuItem
      ...
      {archiveDialogOpen && (
        <ConfirmDialog
          title={dictionary.deposit.archive.confirmTitle}
      ...
      {restoreDialogOpen && (
        <ConfirmDialog
          title={dictionary.deposit.restore.confirmTitle}
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.deposit.destroy.confirmTitle}
