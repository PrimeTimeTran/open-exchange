...
async function goToTradeForm(page: Page) {
  ...
  // TODO: Create trade here
  // await prisma.trade.create({});

  await page.waitForURL('/');
  await page.goto(`/trade/${trade.id}/edit`);
  await page.waitForURL(`/trade/${trade.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToTradeForm(page);
});
