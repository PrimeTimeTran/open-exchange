...
import { tradeFilterFormSchema } from 'src/features/trade/tradeSchemas';
...
function TradeListFilter({
  ...
  const previewRenders = {
    priceRange: {
      label: dictionary.trade.fields.price,
      render: dataTableFilterRenders(context).decimalRange(),
    },
    quantityRange: {
      label: dictionary.trade.fields.quantity,
      render: dataTableFilterRenders(context).decimalRange(),
    },
    archived: {
  ...
  const filter = useMemo(
    () =>
      DataTableQueryParams.getFilter<z.infer<typeof tradeFilterFormSchema>>(
        searchParams,
        tradeFilterFormSchema,
      ),
    [searchParams],
  );

  const form = useForm({
    resolver: zodResolver(tradeFilterFormSchema),
    mode: 'onSubmit',
          ...
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.trade.fields.price}</FormLabel>
                    <RangeInput
                      type="text"
                      dictionary={dictionary}
                      disabled={isLoading}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.trade.fields.quantity}</FormLabel>
                    <RangeInput
                      type="text"
                      dictionary={dictionary}
                      disabled={isLoading}
