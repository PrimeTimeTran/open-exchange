...
import { tradeDestroyManyApiCall, tradeArchiveManyApiCall, tradeRestoreManyApiCall } from 'src/features/trade/tradeApiCalls';
...
export function TradeActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.tradeUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return tradeDestroyManyApiCall([trade.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.trade.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      return tradeArchiveManyApiCall([trade.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['trade'],
      ...
      toast({
        description: dictionary.trade.archive.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      return tradeRestoreManyApiCall([trade.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['trade'],
      ...
      toast({
        description: dictionary.trade.restore.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/trade/${trade.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/trade/${trade.id}/edit`} prefetch={false}>
          ...
          {hasPermissionToArchive && !trade?.archivedAt && (
            <DropdownMenuItem
          ...
          {hasPermissionToRestore && Boolean(trade?.archivedAt) && (
            <DropdownMenuItem
      ...
      {archiveDialogOpen && (
        <ConfirmDialog
          title={dictionary.trade.archive.confirmTitle}
      ...
      {restoreDialogOpen && (
        <ConfirmDialog
          title={dictionary.trade.restore.confirmTitle}
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.trade.destroy.confirmTitle}
