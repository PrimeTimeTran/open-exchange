...
import {
  tradeDestroyManyApiCall,
  tradeFindManyApiCall,
  tradeArchiveManyApiCall,
  tradeRestoreManyApiCall,
} from 'src/features/trade/tradeApiCalls';
import { tradeExporterMapper } from 'src/features/trade/tradeExporterMapper';
import { tradeFilterInputSchema } from 'src/features/trade/tradeSchemas';
...
export default function TradeListActions({
  context,
  table,
  count,
  filter,
  ...
  filter: z.input<typeof tradeFilterInputSchema>;
  ...
  const hasPermissionToCreate = hasPermission(permissions.tradeCreate, context);
  ...
  const hasPermissionToImport = hasPermission(permissions.tradeImport, context);
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      csvExporter(
        tradeExporterMapper(data.trades, context),
        dictionary.trade.fields,
        'trades',
      );
      toast({
        description: dictionary.trade.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.trade.destroyMany.noSelection);
      }

      return tradeDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.trade.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.trade.archiveMany.noSelection);
      }

      return tradeArchiveManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.trade.archiveMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.trade.restoreMany.noSelection);
      }

      return tradeRestoreManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.trade.restoreMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <>
      ...
      {archiveManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.trade.archiveMany.confirmTitle}
          description={formatTranslation(
            dictionary.trade.archiveMany.confirmDescription,
            selectedCount,
          )}
      ...
      {restoreManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.trade.restoreMany.confirmTitle}
          description={formatTranslation(
            dictionary.trade.restoreMany.confirmDescription,
            selectedCount,
          )}
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.trade.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.trade.destroyMany.confirmDescription,
            selectedCount,
          )}
