...
import {
  itemDestroyManyApiCall,
  itemFindManyApiCall,
  itemArchiveManyApiCall,
  itemRestoreManyApiCall,
} from 'src/features/item/itemApiCalls';
import { itemExporterMapper } from 'src/features/item/itemExporterMapper';
import { itemFilterInputSchema } from 'src/features/item/itemSchemas';
...
export default function ItemListActions({
  context,
  table,
  count,
  filter,
  ...
  filter: z.input<typeof itemFilterInputSchema>;
  ...
  const hasPermissionToCreate = hasPermission(permissions.itemCreate, context);
  ...
  const hasPermissionToImport = hasPermission(permissions.itemImport, context);
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      csvExporter(
        itemExporterMapper(data.items, context),
        dictionary.item.fields,
        'items',
      );
      toast({
        description: dictionary.item.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.item.destroyMany.noSelection);
      }

      return itemDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.item.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.item.archiveMany.noSelection);
      }

      return itemArchiveManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.item.archiveMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.item.restoreMany.noSelection);
      }

      return itemRestoreManyApiCall(ids);
    },
    onSuccess: () => {
      table.resetRowSelection();
      queryClient.resetQueries({
      ...
      toast({
        description: dictionary.item.restoreMany.success,
      });
    },
    onError: (error: any) => {
      toast({
    ...
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/item/importer`} prefetch={false}>
      ...
      {hasPermissionToCreate && (
        <Button
          ...
          asChild
        >
          <Link href={`/item/new`} prefetch={false}>
      ...
      {archiveManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.item.archiveMany.confirmTitle}
          description={formatTranslation(
            dictionary.item.archiveMany.confirmDescription,
            selectedCount,
          )}
      ...
      {restoreManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.item.restoreMany.confirmTitle}
          description={formatTranslation(
            dictionary.item.restoreMany.confirmDescription,
            selectedCount,
          )}
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.item.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.item.destroyMany.confirmDescription,
            selectedCount,
          )}
