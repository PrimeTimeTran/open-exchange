...
import { itemDestroyManyApiCall, itemArchiveManyApiCall, itemRestoreManyApiCall } from 'src/features/item/itemApiCalls';
...
export function ItemActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.itemUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return itemDestroyManyApiCall([item.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.item.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      return itemArchiveManyApiCall([item.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['item'],
      ...
      toast({
        description: dictionary.item.archive.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      return itemRestoreManyApiCall([item.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['item'],
      ...
      toast({
        description: dictionary.item.restore.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/item/${item.id}/edit`} prefetch={false}>
          ...
          {hasPermissionToArchive && !item?.archivedAt && (
            <DropdownMenuItem
          ...
          {hasPermissionToRestore && Boolean(item?.archivedAt) && (
            <DropdownMenuItem
      ...
      {archiveDialogOpen && (
        <ConfirmDialog
          title={dictionary.item.archive.confirmTitle}
      ...
      {restoreDialogOpen && (
        <ConfirmDialog
          title={dictionary.item.restore.confirmTitle}
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.item.destroy.confirmTitle}
