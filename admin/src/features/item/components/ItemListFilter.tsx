...
import { itemFilterFormSchema } from 'src/features/item/itemSchemas';
...
function ItemListFilter({
  ...
  const previewRenders = {
    name: {
      label: dictionary.item.fields.name,
    },
    caption: {
      label: dictionary.item.fields.caption,
    },
    description: {
      label: dictionary.item.fields.description,
    },
    priceRange: {
      label: dictionary.item.fields.price,
      render: dataTableFilterRenders(context).decimalRange(),
    },
    category: {
      label: dictionary.item.fields.category,
  ...
  const filter = useMemo(
    () =>
      DataTableQueryParams.getFilter<z.infer<typeof itemFilterFormSchema>>(
        searchParams,
          ...
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.item.fields.name}</FormLabel>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.item.fields.caption}</FormLabel>
                    <Input disabled={isLoading} {...field} />
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.item.fields.description}</FormLabel>
                    <Input disabled={isLoading} {...field} />
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.item.fields.price}</FormLabel>
                    <RangeInput
                      type="text"
                      dictionary={dictionary}
                      disabled={isLoading}
