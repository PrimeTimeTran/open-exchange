...
import { balanceSnapshotDestroyManyApiCall, balanceSnapshotArchiveManyApiCall, balanceSnapshotRestoreManyApiCall } from 'src/features/balanceSnapshot/balanceSnapshotApiCalls';
...
export function BalanceSnapshotActions({
  mode,
  balanceSnapshot,
  context,
}: {
  ...
  const hasPermissionToEdit = hasPermission(permissions.balanceSnapshotUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return balanceSnapshotDestroyManyApiCall([balanceSnapshot.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.balanceSnapshot.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const archiveMutation = useMutation({
    mutationFn: () => {
      return balanceSnapshotArchiveManyApiCall([balanceSnapshot.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['balanceSnapshot'],
      ...
      if (mode === 'view') {
        router.push('/balanceSnapshot');
      }

      toast({
        description: dictionary.balanceSnapshot.archive.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const restoreMutation = useMutation({
    mutationFn: () => {
      return balanceSnapshotRestoreManyApiCall([balanceSnapshot.id]);
    },
    onSuccess: () => {
      queryClient.resetQueries({
        queryKey: ['balanceSnapshot'],
      ...
      toast({
        description: dictionary.balanceSnapshot.restore.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/balance-snapshot/${balanceSnapshot.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/balance-snapshot/${balanceSnapshot.id}/edit`} prefetch={false}>
          ...
          {hasPermissionToArchive && !balanceSnapshot?.archivedAt && (
            <DropdownMenuItem
          ...
          {hasPermissionToRestore && Boolean(balanceSnapshot?.archivedAt) && (
            <DropdownMenuItem
      ...
      {archiveDialogOpen && (
        <ConfirmDialog
          title={dictionary.balanceSnapshot.archive.confirmTitle}
      ...
      {restoreDialogOpen && (
        <ConfirmDialog
          title={dictionary.balanceSnapshot.restore.confirmTitle}
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.balanceSnapshot.destroy.confirmTitle}
