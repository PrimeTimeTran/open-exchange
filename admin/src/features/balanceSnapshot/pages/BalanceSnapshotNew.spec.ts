...
async function goToBalanceSnapshotNew(page: Page) {
  ...
  await page.goto(`/balance-snapshot/new`);
  await page.waitForURL(`/balance-snapshot/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToBalanceSnapshotNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.balanceSnapshot.new.title }),
  ).toBeTruthy();
});
