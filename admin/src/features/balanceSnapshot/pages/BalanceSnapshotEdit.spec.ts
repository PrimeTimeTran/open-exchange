...
async function goToBalanceSnapshotForm(page: Page) {
  ...
  await page.goto(`/balance-snapshot/${balanceSnapshot.id}/edit`);
  await page.waitForURL(`/balance-snapshot/${balanceSnapshot.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToBalanceSnapshotForm(page);
});
