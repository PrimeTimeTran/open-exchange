...
async function createBalanceSnapshot(context: AppContext) {
...
describe('balanceSnapshotAutocomplete', () => {
  ...
  describe('validation', () => {
    it('must be signed in', async () => {
      try {
        await balanceSnapshotAutocompleteController({}, await testContext());
        fail();
      } catch (error) {
  ...
  it.skip('filters', async () => {
    ...
    const data = await balanceSnapshotAutocompleteController(
      { search },
      await testContext({
        currentUserId: currentUser?.id,
        currentTenantId: currentTenant?.id,
