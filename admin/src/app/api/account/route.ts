...
import { accountDestroyManyController } from 'src/features/account/controllers/accountDestroyManyController';
import { accountFindManyController } from 'src/features/account/controllers/accountFindManyController';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const payload = await accountCreateController(body, context);
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const payload = await accountFindManyController(query, context);
    return NextResponseSuccess(request, context, payload);
  } catch (error: any) {
    return NextResponseError(request, context, error);
  }
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    context = await appContext(request);
    const query = queryToObject(request.nextUrl.search);
    await accountDestroyManyController(query, context);
    const payload = true;
    return NextResponseSuccess(request, context, payload);
  } catch (error: any) {
    return NextResponseError(request, context, error);
