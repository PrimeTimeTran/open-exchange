...
import { feedbackUpdateController } from 'src/features/feedback/controllers/feedbackUpdateController';
...
export async function GET(
  ...
  try {
    ...
    const payload = await feedbackFindController(params, context);
...
export async function PUT(
  ...
  try {
    ...
    const payload = await feedbackUpdateController(params, body, context);
    return NextResponseSuccess(request, context, payload);
  } catch (error: any) {
    return NextResponseError(request, context, error);
  }
